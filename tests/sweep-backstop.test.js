...
test('tidy/SKILL.md Step 7 documents the marker and grant, and honestly states Step 7 does not yet open a PR', () => {
  assert.match(TIDY_SKILL, /<!-- tidy-housekeeping-pr -->/);
  assert.match(TIDY_SKILL, /`housekeeping-auto-merge` set project-wide/);
  assert.match(TIDY_SKILL, /As of this writing, Step 7 above does not itself open a PR/);
});
