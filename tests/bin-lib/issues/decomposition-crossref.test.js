...
test('a short generic sibling title that appears only as a substring of another word does not wrongly cross-reference', () => {
  const bodyA = '## Gotchas\n\n- This uses a prefix convention with `helper()`.\n\n### Key Files\n\n- `a.js`\n';
  const bodyB = '## Deliverables\n\nBuild it.\n\n### Key Files\n\n- `b.js`\n';
  const units = [
    { title: 'Origin task', body: bodyA },
  ...
  assert.deepStrictEqual(grepCalls, [], 'grep should never run for a title that only appears as a substring inside another word');
  assert.deepStrictEqual(result, []);
});
