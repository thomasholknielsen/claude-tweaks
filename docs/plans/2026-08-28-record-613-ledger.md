...
| 6 | wrap-up | Reflect (light, Friction): `hooks.js log-decision` silently no-ops instead of erroring on unrecognized subcommand. Self-reference check collapsed D5 (this project IS claude-tweaks) — re-classified as an in-project tangential backlog candidate. | deferred | Filed as #1586 (queueWriteAutoFile, unattended) |
