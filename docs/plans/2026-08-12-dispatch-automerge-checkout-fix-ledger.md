...
| 3 | wrap-up | worktree — `.claude/worktrees/policy-read-path-design` (locked, branch `worktree-policy-read-path-design`) — `git worktree list --porcelain` | accepted | Lock confirmed stale (owning pid 24015 no longer running) and branch tip confirmed an ancestor of main, but `git worktree remove` refuses a locked worktree — per `[IL-58]` this is the SessionStart reaper's job, not a manual force-unlock. Left for the reaper. |
