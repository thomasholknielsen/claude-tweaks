...
- **Per-item loops override the multi-item batch table** — a skill whose procedure iterates a human-supplied list and collects one *human judgment* per element (`/demo`'s per-record verdict, `/specify` shaping mode's per-record design-intent question) does **not** batch. The Interaction style directive's batch table is for pre-filled *recommendations* the human confirms; when the per-item decision **is** the judgment being collected, there is nothing to pre-fill, and one table would ask for N verdicts in a single answer. State the exception in the skill's own `## Input`, in one sentence naming the directive (both skills above do). Never edit the directive line itself to carve the exception out — it is pinned byte-for-byte across every SKILL.md by `tests/bin-lib/skill-audit/house-structure.test.js`, so the precedence clause always lives in the skill body.
...
- **`argument-hint`** — required whenever a skill's `## Input` section documents any accepted argument grammar; omit only for a skill that genuinely takes no arguments at all — none currently do (every skill in this plugin declares a non-empty `argument-hint`), but the convention still applies if one is ever added. Shows as greyed-out placeholder text in the terminal when the user types `/claude-tweaks:{skill}` — purely cosmetic, has no effect on how `$ARGUMENTS`/`$1`/`$2` are parsed at runtime. Always quote the value (`"..."`, or `'...'` when the hint itself contains a literal `"`) — an unquoted value starting with `[` is invalid YAML (parsed as a flow sequence, not a string). Derive the hint directly from the skill's own `## Input` section — same bracket/pipe convention as `.claude/commands/*.md`: `[optional]`, `<required>`, `a|b` for alternatives. Keep it in sync when `## Input` changes. **When an argument's *arity* changes** (one ref → a `#N,#M` list, one value → a repeatable flag), that sync is repo-wide, not file-scoped: the hint, `## Input`, `skills/help/reference-card.md`, and **every prose cardinality claim anywhere** must move together. Grep the retiring vocabulary — `per invocation`, `one item`, `a single {thing}`, the old bracket form — across `skills/`, `docs/`, and `README.md`, and derive the edit list from those hits rather than from the spec's named deliverables. #695 shipped the hint and the card exactly as its file list specified and still left six live single-ref sentences behind (`demo`'s own `description`, intro, and Anti-Patterns row; `_shared/github-pr-scan-acceptance.md`; `help/status-scan.md`; `_shared/work-record.md`), five caught only at the whole-branch review and one only at wrap-up.
