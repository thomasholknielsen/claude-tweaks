...
- **Action:** Read the JSON envelope's `headroom.composed` rows — one per compose call site whose sources the plan touches, with `max`, `ceiling`, and `over` (those, plus `headroom.composedNearCeiling` and `headroom.composedErrors`, are the envelope keys `plugin/skills/build/plan-audit.md` documents as the multi-spec pre-flight) — and then the grep, which now returns only the plan-audit CLI's two fixture lines (`context-cost.test.js` imports the constant rather than spelling the number).
