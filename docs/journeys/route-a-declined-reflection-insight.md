...
- **Action:** For each insight the five lenses (plus the tradeoff review) produced, `full-mode.md`'s Prior-decline annotation step computes a fingerprint from the insight's own description text (`createFingerprint('reflect', ['description'])`, `bin/lib/health-core/fingerprint.js`) and looks it up in the declined-learning store (`lookupDecline(fingerprint)`, `bin/lib/declined-learning/store.js`). When that exact-hash lookup finds nothing, a subject scan (#1033) also runs: `listDeclined({ source: 'reflect' })` returns every declined entry's human-legible `subject` text (an entry recorded before #1033 has none — skipped), and the agent judges — not a mechanical string match — whether the current insight is materially the same point as any listed subject, even where the wording differs. This is what makes near-miss matching no longer fingerprint-only: a rephrased repeat of a prior decline now annotates too, not just a byte-identical one.
...
- **Should understand:** Nothing is filtered or hidden at this step — every insight still gets a full table row and a real recommendation, matched or not (by either path). The lookup only decides whether to append an annotation, never whether to include the row at all. The subject scan is a judgment call, not a second deterministic lookup — genuine uncertainty means no annotation, not a forced match.
- **Red flags:** A previously-declined insight silently dropped from the table instead of rendered with its annotation; either lookup running against live text instead of the frozen description that was actually declined, producing a match that shouldn't exist; the subject scan annotating on a superficial wording echo rather than a genuinely material match.
...
- **Action:** For each annotated insight: if it resolves to "Don't capture" again, the decline is (re-)recorded via `recordDecline(fingerprint, { reason, source: 'reflect', subject: description })` (#1033 added `subject` — the insight's own one-line description, so a future run's subject scan has something to compare against) — same fingerprint, fresh `declinedAt`/`reason`/`subject`. If it resolves to anything else (Implement now, Defer, Capture — the maintainer decided the earlier "don't capture" no longer holds), the stale entry is cleared: for an exact-hash match, via `clearDecline(fingerprint)` using that same fingerprint; for a subject-scan match, via `clearDecline` on **the matched `listDeclined` entry's own fingerprint**, not one freshly computed from the current insight's differently-worded text — that fresh fingerprint isn't a key in the store, so clearing it would silently no-op.
...
- **Red flags:** A re-declined insight's `declinedAt`/`reason` not updating (stale timestamp implies the store isn't actually being re-written); a captured insight's annotation still showing up on the very next run because `clearDecline` wasn't called, or was called on the wrong (freshly-computed) fingerprint for a subject-scan match.
...
- Updated for #1033 (this journey's core premise: near-miss matching is no longer fingerprint-only — a subject scan over `listDeclined` now judges reworded repeats too, alongside the store's new `subject` field)
