...
- **URL:** no page — an `AskUserQuestion` rendered by the agent per `plugin/skills/dispatch/SKILL.md`'s "Confirm before resuming": header `Resume run`, question opening `Resume {target} toward merge? PR #{number} …`, and exactly two options — `Resume (Recommended)` and `Cancel`.
