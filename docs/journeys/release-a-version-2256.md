...
- **Should understand:** Five outcome words, never folded: `released`, `dry-run`, `HELD` (a train gate fired, nothing moved), `PARTIAL` (the release exists, something after it did not land), `failed` (Step 5 attempted and landed nothing). Under `local-merge` the engine's own exit code is the verdict (`5` means the tag is final and only the hook failed — re-run the hook, never the release; `1` is either nothing written or a named partial state, and its stderr says which). `--as` is pr-first only: on a `local-merge` repo it is refused right after the preflight pack, before any review runs.
