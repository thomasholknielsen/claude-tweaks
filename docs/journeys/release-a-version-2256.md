...
- **Action:** Steps 1–4 as before; Step 5 merges release-please's open PR with `gh pr merge --squash` (or, on a `local-merge` repo, runs `bin/release-local.js`); Step 6 waits — bounded — for the tag on origin, the GitHub Release and the `release: published` workflow run, and names any miss as a partial state with its recovery command; Step 7 comments `Shipped in v1.3.0` on each shipped record and closes it.
...
- **Should understand:** Under `local-merge` the engine's own exit code is the verdict (`5` means the tag is final and only the hook failed — re-run the hook, never the release).
- **Red flags:** `released` printed while the publish workflow is still running or failed; a record closed without the `Shipped in` comment; `--as 7.0.0` accepted without the major-bump gate.
