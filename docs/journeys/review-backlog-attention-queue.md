...
- Updated during build of #967/#968/#969 — `/specify next` (the headless shaping unit) introduced `shaped:headless`, `/backlog attention` gained it as a third classification, and `grant-gate.js` gained the `shaped-headless-floor` deny that makes this list the sole surface for a machine-shaped record above the provenance floor.
