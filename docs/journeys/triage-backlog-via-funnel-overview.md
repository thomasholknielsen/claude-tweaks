...
- **Action:** On a repo where the `needs:definition` label family exists, read the `└─ needs you: {n}` branch line under the funnel header and the `── Needs you ──` section rendered last, just above the Next Actions block.
- **Should feel:** Like the report handing you *your* work: paste blocks send agents off; the lane's launcher lines (`/claude-tweaks:specify #N` for definition gaps, `/claude-tweaks:challenge` for evidence calls once #471's form lands) are the session's recommended move whenever the lane is non-empty.
- **Should understand:** The lane is an overlay (`funnelBuckets`' `needsYou`) — records keep their stage bucket and are counted twice by design; only in-play records surface (in-flight/parked/not-planned are filtered); on a repo without the labels nothing renders at all (dormant-safe).
