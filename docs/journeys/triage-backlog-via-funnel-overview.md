...
**Success state:** A funnel header whose six stage counts, together with the parked/not-planned annotation counts and the parents count in its own Sign-off paste block, account for the whole open queue, at most two annotation lines beneath the header, a "Recommended next" callout, and per-stage paste blocks with integrity rules and a closing `Next:` line — with no lens tables rendered and every record appearing exactly once.
...
- **Red flags:** More than two annotation lines; a `parents N → ...` line rendered beneath the header (it moved to the Sign-off paste block); a rendered trust table in bare mode; a consequence line for an `insufficient-evidence` cell; a trust verdict rendered after the fetch reported a failed parent.
