...
**Success state:** A funnel header whose six stage counts, together with the parked/not-planned and parents annotation counts, account for the whole open queue, at most three annotation lines beneath it, a "Recommended next" callout, and per-stage paste blocks with integrity rules and a closing `Next:` line — with no lens tables rendered and every record appearing exactly once.
