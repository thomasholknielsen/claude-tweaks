...
- **Should understand:** `--run-dir` accepts either the run's original path or (the normal case — archival precedes verification) its archived copy under `.claude-tweaks/pipelines/archive/`; the verb resolves both. `--base` is consumed only by `carrier-commit` and `reference-repairs`, the two checks that walk `{base}..HEAD`. `plans-ledger`/`design-caches` scan the checkout the verb is actually invoked *from* (`cwd`), never only the main checkout (`repoRoot`) — under this project's default `worktree`/`pr-first` mode a leftover the run itself created lives in the worktree's own working tree, invisible from the main checkout even though both share one git object store (#1222). The blind spot only fully closes when the worktree still stands at verify time — a multi-spec run's `MULTISPEC_REVIEW_DEFER=1`, `cleanup-only` mode, or a manual invocation before teardown; on a fully-torn-down single-spec run `cwd` and `repoRoot` coincide (the worktree is gone) and this distinction is moot.
- **Red flags:** Any check silently reporting `pass` for a condition that plainly doesn't hold on disk — the exact failure class four separate fix rounds in this feature's own build caught (a match-key bug, a vacuous slug match, an ISO-timestamp-stripping regression, and #1222's repoRoot-only blind spot to worktree-local leftovers) before this journey's own files reached the state they're in now.
...
- Related: #343 (prerequisite — fixed the same skill file's circular gate condition on the same branch), #892/#891 (the sibling `bin/verify.js` deterministic-check-runner family — a different subsystem covering type/lint/test checks, not this feature's closure-gate checks), #1222 (updated — closed `plans-ledger`/`design-caches`'s structural blind spot to worktree-local leftovers under the default `worktree`/`pr-first` mode, per record #900's own whole-branch re-review finding #3).
