...
**Structural guarantee, not a documented convention.** `bin/lib/harness-health/scope.js`'s `listTargets(root)` aggregates `listSkills` + `listRules` + `listClaudeMd` + `listDesignArtifacts` — the pool a bare `next-target` call (no `--kind`) rotates over, which is exactly what `routine-template.yml`'s headless firing issues (`prompt: "/claude-tweaks:harness-health"`, no arguments at all). A new `listMemory(memoryDir)` function is added but **never merged into `listTargets`'s aggregate** (`listTargets` still only takes `root` — a memory directory is a different, explicitly-supplied path, not something derived from `root`). The CLI only calls it via a separate branch, taken exclusively when `--kind memory` is explicitly the requested kind. This means a bare Routine firing cannot select a memory target by construction, regardless of whether the Routine's execution sandbox could even reach `~/.claude/projects/{slug}/memory/` — the exclusion holds even if that filesystem constraint ever changed. This fulfills the "still support memory tweaks when running locally" requirement: `/claude-tweaks:harness-health --kind memory` (optionally with `--target <name>`) is the explicit, human-typed, interactive-only entry point.

**Memory root is a passed-in parameter, never derived.** `~/.claude/projects/{slug}/memory/`'s slug is Claude Code's own internal naming scheme — undocumented anywhere in this codebase, with no environment variable exposing it (confirmed: no `CLAUDE_*` env var carries it) and no existing helper reimplementing it. Deriving it from `root` would mean guessing a private implementation detail that could silently change upstream, and it would also have to account for `root` being a worktree path (this project runs under `worktree.always`, so every real invocation happens from inside one) rather than the main checkout the memory directory is actually keyed to. Both problems disappear the same way: don't derive it at all. The invoking assistant already knows its own memory directory's exact path — it is stated verbatim in the system prompt's auto-memory section at the start of every session — so `--kind memory` takes a required `--memory-dir <path>` argument instead, and `listMemory`/`selectMemoryTarget` accept that path as a plain parameter. `harness-health/SKILL.md` instructs the assistant to supply it from what it already knows, never to compute it.

**Selection.** `listMemory(memoryDir)` enumerates `MEMORY.md`'s entries at the given path as target candidates (`{ kind: 'memory', id, path }`). A parallel `selectMemoryTarget` runs only Phase 1 of `selectTarget`'s existing algorithm (force-pick anything unaudited past `STALE_DAYS`) — no Phase 2 hotspot/churn scoring, since memory has no git signal, matching the original design doc's own reasoning. `why` is therefore always `stale` or `manual`, never `hotspot`.
