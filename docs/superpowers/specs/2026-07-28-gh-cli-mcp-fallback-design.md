...
| `bin/lib/health-core/durable-state.js` | `writeState`'s blob/tree/commit/ref sequence gets a sibling MCP-shaped path: `create_or_update_file`/`push_files` carrying the expected `sha`, with the same `MAX_CAS_ATTEMPTS`/backoff retry loop on a sha-mismatch response. The `command -v gh` check runs once per module load and is cached in a module-level variable (mirroring how the module already caches nothing per-call today — this adds the first such cache), not re-shelled on every `writeState` invocation. Read path (`readState`) is untouched — already `gh`-free. |
...
**Health-state cursor write, MCP path (new):** `git fetch`/`git show` still runs unmodified (already `gh`-free) to read current state and each file's current `sha` → mutate in memory (unchanged) → `create_or_update_file`/`push_files` per changed file, each carrying that file's `sha` from the read above → a sha-mismatch response is the retry trigger, same backoff loop, same `MAX_CAS_ATTEMPTS` — just a different failure signal to detect.
...
**Ambiguous-failure disambiguation carries over structurally.** Today's `writeState` doesn't blindly retry every PATCH failure — it first checks whether the update actually landed (`currentCommitSha(root) === commitSha`) before deciding to retry, since a network blip can make a successful write look like a failure. The MCP path needs the identical discipline: after an ambiguous `create_or_update_file` error (timeout, connection drop — not a clean sha-mismatch response), re-read the file's current `sha`/content and compare against what was intended before retrying.

**Two distinct failure classes.** A sha-mismatch (someone else wrote first) is the expected, bounded-retry case — same `MAX_CAS_ATTEMPTS`/backoff loop as today. A hard tool error (the MCP call itself fails) must not be treated as a CAS-retry signal — retrying into a broken transport just burns the attempt budget without ever succeeding.
...
- **`durable-state.js`:** the module already shells out via an injectable seam for its existing tests — the MCP path gets the same treatment, an injectable MCP-call function so tests can simulate: a clean success, a sha-mismatch (retry trigger), an ambiguous failure requiring disambiguation, and a hard tool error (must not retry as a CAS conflict). Read-path tests are untouched.
