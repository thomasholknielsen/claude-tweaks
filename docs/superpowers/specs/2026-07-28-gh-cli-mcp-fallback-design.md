...
| `bin/lib/health-core/durable-state.js` | `writeState`'s blob/tree/commit/ref sequence gets a sibling MCP-shaped path: `create_or_update_file`/`push_files` carrying the expected `sha`, with the same `MAX_CAS_ATTEMPTS`/backoff retry loop on a sha-mismatch response. Selected once per process via the `command -v gh` check. Read path (`readState`) is untouched — already `gh`-free. |
