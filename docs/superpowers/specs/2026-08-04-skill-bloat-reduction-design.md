...
| Interaction-style directive | 571 B × 32 files = 17 KB | State once in CLAUDE.md | ~16 KB |
| ASCII lifecycle diagrams | ~14 KB (approximate) | Canonical copy in `/help` only | ~14 KB |

**Total ~150–170 KB, roughly 16–18% of `SKILL.md` bytes.**
...
4. **Same-change-set dependencies** (`[IL-02]`, `[IL-60]`), both re-pointed in Phase 2, not
   after: `skills/_shared/auto-mode-contract.md`'s rule that each pipeline-participating skill
   "MUST reference this file in its Relationship table", and
   `skills/harness-health/library-shape-analysis.md`'s overlap check, which treats a
   documented Relationship entry as evidence against skill overlap.
...
Risk-ascending. Phase 1 first is deliberate: it banks ~30 KB with zero judgment calls and
...
| **1** | Hoist the interaction directive into CLAUDE.md and drop 32 copies; remove ASCII lifecycle diagrams; update CLAUDE.md conventions and the structural tests | **None** — mechanical, ~30 KB |
...
| Cross-skill awareness degrades once the tables are gone | `docs/skill-graph.md` retains every edge; `/harness-health`'s overlap check re-points to it in the same change-set |
