...
3. Return `{ kind: 'design-artifact', id: 'PRODUCT' | 'DESIGN', path, domainPaths }`:
   - `id: 'PRODUCT'` → `domainPaths: []` (no churn proxy — see Architecture rationale in Problem section; Phase 2 of `selectTarget` already skips any candidate with zero churn, so this candidate is only ever picked via Phase 1's flat staleness timer).
   - `id: 'DESIGN'` → `domainPaths` = the same frontend-signal git pathspecs `/init`'s bootstrap already uses for frontend detection (`*.tsx`, `*.jsx`, `*.vue`, `*.svelte`, `*.css`, `components/`, `pages/`, `app/`, `routes/`, `views/`, `ui/`) — reuses `domainChurn()`'s existing glob-pathspec support unchanged (the function comment already documents it accepts "exact file paths... or glob pathspecs").
...
2. Build `oldString` from `target.why`: `"Unaudited for {daysSinceLastAudit} days"` (stale) or `"{churnCount} commits touching {domainPaths joined}, since last audit"` (hotspot).
...
- `bin/lib/harness-health/tests/scope.test.js` — new cases for `listDesignArtifacts`: flag-gated (missing/disabled/plugin-only/enabled), canonical-path resolution, fallback-path resolution, `PRODUCT` gets empty `domainPaths`, `DESIGN` gets the frontend-signal glob list, both feed into `listTargets()`'s aggregation.
