...
7. **Verify (Step 6)** under pr-first: tag on origin (`git ls-remote --tags origin "v{version}*"`, keeping only the lines whose ref is exactly `refs/tags/v{version}` or `refs/tags/v{version}^{}` — the `*` glob is what makes an annotated tag print its peeled ref, which probe 3 needs for the commit sha, and the exact-ref filter is what keeps `v{version}0`-style neighbours out), `gh release view v{version}`, then the `release: published` workflow run via `gh run list --event release --json status,conclusion,name,url --limit 20`, bounded 15 × 20 s — missing after the bound, or `conclusion` not `success`, is the named partial state; under local-merge the engine's own exit code is the verdict (`5` = hook failed after the tag; `1` = a named partial state, quoted verbatim).
...
14. **Gating version vs shipped version** — Step 4 gates on `--as` or `proposedVersion`; Steps 6–7 verify and book the engine's own version (`release-local.js`'s `released v{version}` stdout line; the merged release PR's `chore(main): release X.Y.Z` title). The pack never fetches and release-please reads config the pack does not, so the two can differ; a difference is logged and the shipped version wins, except under `--as` where a mismatch is `failed`.
15. **The release PR's own checks gate the merge** — `execute.md` reads `gh pr view {n} --json state,mergeStateStatus,statusCheckRollup` and classifies per `_shared/pr-first-merge.md` Step 2.5 against the resolved `merge-verification` lever: red → `failed` naming the checks; pending past the bound → `failed` (the release never arms `--auto`, because Step 6 must verify inside this run); green, or lever `off` → merge. `ciTip` is the integration-branch tip, not the PR, and the console's CI row renders — it never gated.
16. **Engine exit `1` splits on its stderr prefix** — `release-local: … — nothing written` → `failed`; `partial: …` → `PARTIAL`. The no-origin fallback keys on `git remote get-url origin` failing, never on a stdout string. `--dry-run` under local-merge invokes the engine with `--dry-run` and renders its output (manifest paths, hook, drift warning, changelog section) after the console — the console does not carry those.
17. **The `--as` refusal under local-merge is decided in Step 1**, right after the pack is read (SKILL.md owns it; `execute.md` keeps a back-stop). The refusal logs as `SKIP`, not `REFUSED` (which the console renders as an unresolvable queue-write row). The `Release-As:` push is anchored with `git -C "$RUN_ROOT"` on `{branch}` and a rejected push is `failed`, not a poll.
18. **Sub-files are actionable by construction** — the reference-form scanner reads every `plugin/skills/release/*.md` other than `SKILL.md` whole, exempting only the preamble before the first `## ` heading and a `## Anti-Patterns` section; `SKILL.md` keeps the `## Step…` / `## Next Actions` rule. A heading whitelist (`Step`, `Next Actions`) left `console.md` and `bookkeeping.md` unscanned — a lazy-loaded sub-file is read only when its step executes, so all of it is instruction text.
...
19. **`HELD` is mode-independent** — it means a HARD-GATE fired at Step 4 before Step 5, in any mode, and nothing moved. `release-held.md` is staged in every mode: under `--train`/headless/auto it is the only record of why, and an interactive `Stop` answer stages the identical file with the summary reading `HELD`. An interactive `Proceed` continues to Step 5. `failed` keeps meaning "Step 5 attempted, nothing landed".
20. **Step 7 runs only when the tag is on origin** (or, under local-merge in a repository with no `origin` remote, exists locally). The local-merge exit-`1` `partial:` state — committed and tagged locally, not pushed — is therefore a skip: Step 6 did not run, the recovery push comes first, and the records are booked by the re-run after it. Booking a tag the forge cannot corroborate is the failure this avoids.
21. **The standalone mint is registered** — `/claude-tweaks:release` joins `_shared/pipeline-run-dir.md`'s step-4 standalone-auto allowlist, and its `createdBy: "release-standalone"` stamp is named there as the second direct `run-state.json` write alongside wrap-up's. Step 0 determines inherited-vs-created once and carries it as a run fact; the Component-Skill Contract keys Next Actions on that fact (created → render, inherited → omit), and Step 3 exporting `$PIPELINE_RUN_DIR` for the nested `/claude-tweaks:review` invocation never changes it.
