# Parse-Signal Discipline Skill Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a new project-local skill documenting the rule that a mechanized deterministic
check parsing loose, real-world prose must signal "couldn't parse this" as a value distinguishable
from "this input legitimately doesn't apply" — never collapse both into one silent `null`/skip.

**Architecture:** A single new maintainer-facing convention skill at
`.claude/skills/parse-signal-discipline/SKILL.md`, following the same frontmatter + plain-markdown
shape as the existing project skills in that directory (e.g.
`.claude/skills/transcript-payload-verification/SKILL.md`). No code changes — this is a
documentation-only deliverable; nothing in `plugin/` or `bin/` is touched, so no `node --test`
suite is affected.

**Tech Stack:** Markdown (YAML frontmatter + prose), no code.

**Spec:** `.claude-tweaks/pipelines/2026-09-05T164740-record-1595/work/1595-spec.md` (record #1595)

## Global Constraints

- New skill lives under `.claude/skills/` (project-local convention skill), never under
  `plugin/skills/` — this is a maintainer-authoring rule, not a shipped user-facing skill.
- Frontmatter `description` must end with a `Keywords - ...` clause, matching every sibling skill
  in `.claude/skills/`.
- The worked example must cite the real function name `extractStep2Verification`
  (`plugin/bin/lib/plan-audit/parser.js:93`), not the record's stale `parseStep2` name — the
  record body itself has drifted (materialize.md's Named-location drift case): `parseStep2` does
  not exist anywhere in the repo. This plan corrects that citation; the skill file must correct it
  too, and should note the correction so a future reader isn't sent looking for a function that
  isn't there.

---

### Task 1: Author `.claude/skills/parse-signal-discipline/SKILL.md`

**Files:**
- Create: `.claude/skills/parse-signal-discipline/SKILL.md`

**Interfaces:**
- Consumes: nothing (first and only task).
- Produces: the skill file itself — the deliverable. No code symbols.
...
- [ ] **Step 2: Verify the deliverable (read-only check, not a Run/Expected:FAIL pair)**

This is a new-content task, not a bugfix — there is no pre-existing failing behavior to reproduce,
so Check C's own Run:/Expected:FAIL convention doesn't apply here. Verify by reading the file back
and confirming both required signals are present:

Run: `grep -c "extractStep2Verification" .claude/skills/parse-signal-discipline/SKILL.md && grep -c "couldn't parse\|could not parse" .claude/skills/parse-signal-discipline/SKILL.md`
Expected: both greps return a non-zero count — the corrected worked-example citation and the
could-not-parse anti-pattern language are both present in the file.

- [ ] **Step 3: Commit**

```bash
git add .claude/skills/parse-signal-discipline/SKILL.md
git commit -m "Add parse-signal-discipline skill — deterministic prose checks must signal parse failure distinctly from not-applicable"
```
