...
Fetch and facet-parse the full open-issue queue per `_shared/record-queue-fetch.md`, same as `refine-mode.md`'s priority/Related fetch (`{tmp-records-file}` = `/tmp/backlog-overview-open.json`, `{tmp-faceted-file}` = `/tmp/backlog-overview-faceted.json`, `{EXTRA_FIELDS}` = `,body` — explicitly restated here, not just inherited by implication: Step 3 below needs every candidate's `body` for `rankNextToBuild`'s internal `parseDependencies` call, and without this field the fetch would silently omit bodies, making every candidate's unblocks-count silently compute as 0 rather than erroring). **Under `work-backend: github-issues`**, also fold in `unsynced: true` local fallback records the same way (port `review-backlog/SKILL.md`'s old Step 1 unsynced fold-in verbatim) — matching `refine-mode.md`'s own Step 1 qualifier and the source's own driver-scoped structure (`unsynced` only exists as a failed-GitHub-write artifact under `local-files`; the fold-in has no meaning there). Tag every fetched record with a **not yet synced** marker wherever `facets.unsynced === true` — this is a display-only tag in `overview` mode; the apply path for unsynced records' priority lives in `refine` mode's Apply step (writing `priority:*` via `writeRecord` when a record has no `$ISSUE` — see this plan's Global Constraints correction note).
...
```markdown
**`/claude-tweaks:backlog`** — Two modes over the open work-record backlog, both always interactive (no headless mode). `refine` ensures every record carries the right `priority:*`/`**Related:**`/grant labels: accepts an optional `--origin <code-health|harness-health|journey-health|docs-health|capture|human>` filter narrowing the worklist, pulls ungranted `ready` records, gets a content-aware recommendation from `/claude-tweaks:assess-agent-autonomy`'s `grant-check` mode (reads the record's actual body, not just its risk/effort labels), re-verifies spec shape before granting (a `ready` label alone never authorizes — the gate re-fetches the body), and — together with a bounded LLM synthesis pass over unscored records producing `priority:*` suggestions and detected `**Related:**` cross-references — applies everything after a single unified batch confirm (a human always executes the actual label write, even when the recommendation is accepted as-is). A record that hit its retry ceiling (`bot:blocked`) surfaces as a re-authorization candidate. `overview` is read-only: mechanical label filtering (`critical`/`risk-value`/`cleanup` lenses) over already-scored records at any backlog scale, plus a "what to build next" recommendation; it also folds in `unsynced: true` local fallback records (surfacing, never fixing them — that stays `/claude-tweaks:tidy`'s job). Neither mode claims, builds, or dispatches anything itself.
```
...
**Actual result when this step ran:** one real match, outside the `skills/` tree this step's own grep otherwise covers — `evals/scenarios/dispatch-local-files-preflight-stop.yaml:5`, a sibling eval scenario never touched by any task in this plan (only `evals/scenarios/triage-permission-matrix-compliance.yaml`, renamed in Task 15, was in scope). Its description cites both a dead path (`skills/triage/SKILL.md`) and a stale scenario name (`the triage-permission-matrix-compliance scenario`, renamed in Task 15 to `backlog-refine-permission-matrix-compliance`). Fix: replace `skills/triage/SKILL.md` with `skills/backlog/SKILL.md`, and `the triage-permission-matrix-compliance scenario` with `the backlog-refine-permission-matrix-compliance scenario`, in that one description paragraph.
...
**Actual result when this step ran:** ~100 hits, all but 5 confirmed legitimate (generic English "triage" as verb/noun, established scope-key names like `triage-queue`, established routine-variant names like `tidy-github-triage`, GitHub's own "Triage" RBAC permission role, legacy config-key aliases like `triage-retry-ceiling`, and already-reviewed deliberate historical/design-rationale mentions from Tasks 5/9/10/11). 5 genuine misses, all pre-existing content untouched by any prior task's line-numbered scope:

| File | Old text | New text |
|---|---|---|
| `skills/backlog/refine-mode.md` (~line 59) | `This mirrors \`triage/SKILL.md\`'s old Step 1 exactly, including the origin-agnostic default and the fresh/blocked split` | `This mirrors the retired \`/claude-tweaks:triage\` skill's old Step 1 exactly, including the origin-agnostic default and the fresh/blocked split` (drops the now-dead `triage/SKILL.md` path reference while keeping the legitimate historical/design-rationale content) |
| `skills/backlog/refine-mode.md` (~line 133) | `using the same cached-body-reuse trick \`triage/SKILL.md\`'s old Step 3.5 used` | `using the same cached-body-reuse trick the retired \`/claude-tweaks:triage\` skill's old Step 3.5 used` (same treatment) |
| `skills/harness-health/SKILL.md` (~line 134) | `` `grant-check` mode (triage's Step 2) recognizes a `new-skill` finding `` | `` `grant-check` mode (`backlog refine`'s grant sub-stage) recognizes a `new-skill` finding `` (matches the established "Step 2" → "grant sub-stage" substitution pattern from Task 10) |
| `skills/dispatch/SKILL.md` (~line 416) | `a prior skill (e.g. triage's Next Actions) invoked it on a human's behalf` | `a prior skill (e.g. \`/claude-tweaks:backlog refine\`'s Next Actions) invoked it on a human's behalf` (missed by Task 10's own line list, which covered lines 19/37/314/412/439/446-447/459 but not this one) |
| `skills/help/context-flow.md` (~line 17, the ASCII diagram's own top label row) | `Work record (durable)             ──→ Triage grants        ──→ Dispatch claims + builds` | `Work record (durable)             ──→ Backlog grants       ──→ Dispatch claims + builds` (missed by Task 13's own line list, which fixed lines 18/33/57 of this same file but not line 17 — the diagram's own label row directly above the already-fixed path/detail row, re-pad spacing to keep the `──→` columns aligned with the row beneath) |
