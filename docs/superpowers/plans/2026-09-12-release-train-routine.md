...
3. **Model and tools** — `claude-sonnet-5`, `[Bash, Read, Grep, Glob, Task]`: every write the train makes is a Bash-invoked CLI or git/gh call (`stage-item.js`, `log-decision.js`, `set-config.js`, `gh pr merge`, `git commit`, `gh issue comment/close`, `node -e closeRecord`), and Step 3's review dispatches lenses via Task — so `Edit`/`Write` are never used and are not granted to an unattended merge-capable firing (review lens 3b; the first draft copied `dispatch`'s set, which needs them only because `/flow` implements code).
...
7. **ACs 2–3 are #2256's `--train` classification invoked verbatim** — proven by the release skill's own journey step 4 (`docs/journeys/release-a-version-2256.md`) and #2256's build-time whole-branch scenario walk (its scenario (e): `--train`, local-merge, breaking commit → `HELD`, recorded in that run's review log, not in the tree); this unit adds no fixture run, exactly as the spec's AC text allows ("exactly as reliable as unit 6's own logic").
8. **A refused `--train` with no human present stops before Step 1** — the reviewer found ruling 4's premise unsupported: a refused train "behaving as on-demand" in a headless firing reached no gate on a clean history and would have shipped through Step 5. `release/SKILL.md`'s Refusal paragraph now splits on human presence (interactive/hybrid → on-demand; `auto` or a mode-less firing → `failed` with the refusal line, nothing read, nothing moved); the autonomy row, the template notes and the routine bullet say the same. `routine-kickoff/SKILL.md`'s hand-maintained manual-execution exclusion list gains `release` (it merges, tags and closes records).
