...
- Produces:
  ...
  - `aggregate({tsv, runs:[{runId, decisionLines:[string], events}], rowIds, windowN}) -> {window:{runIds:[string]}, perRow:{[rowId]:{appearances:number, findings:number}|'no runs in window'}, consoleDist:{[decision]:number, unlogged:number}, reversibilityDist:{high:number,med:number,low:number,'n/a':number}, frictionCounts:{[kind]:number}, refusedCount:number, narrowingSignal:string[], suppressions:{narrowing:string[], ceiling:boolean}}` — `narrowingSignal` is the above-threshold-AND-zero-findings set Task 2's CLI renders as the actual signal line; `suppressions.narrowing` is the separate below-threshold (suppressed) set
