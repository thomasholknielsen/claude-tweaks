...
```markdown
**Independent 2-unit collapse (Step 2.6, `decomposition-mode.md`) — `Related:` cross-links, not parent/child.** When Step 2.6 collapsed two independent units, there is no parent to link either sub-issue to. Instead, each of the two records gets a line-anchored, greppable `Related: #N` body line pointing at the other — `work-backend: github-issues`, both `work-links` values (GitHub's own automatic `#N`-mention timeline cross-reference exists but is not greppable record-body text, which is why this explicit line is written even under `work-links: native`); `work-backend: local-files`, the identical `Related: {id}` body line, no new frontmatter facet. Write this via the same compose-then-write-once discipline as every other body edit in this skill — as a post-create edit inside Step 4's linking pass (both numbers must exist first): create both records in Step 3, then recompose each record's full body with its `**Related:**` line (the repo-canonical bolded form) and write it once per record. [Amended post-review: the original draft here said pre-create, which is impossible — neither number exists yet — and used the unbolded format; the shipped text follows final-review ruling F1/F5.]
```
