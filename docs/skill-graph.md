...
| `/demo` | Step 4.8's `acceptance-gap` scope (`_shared/github-pr-scan.md`) finds closed records with no acceptance label via `needsBackstop` (`bin/lib/issues/acceptance.js`) and recommends `/claude-tweaks:demo #{n}` at severity `info` — staged, never auto-applied at any aggressiveness tier, since disposing a closed record is a human judgment call the auto-mode contract keeps off the silenced list. Those records are by construction other sessions' work, so `/demo`'s Step 1 reconstructs their brief from the closing commit (`git log --grep` over `origin/main` + `HEAD`) rather than from session recall, which would have nothing to recall. Step 4.8's `family-gate` scope (same file) is the sibling backstop for decomposition families: it finds parents whose every leaf has closed but which never reached `/claude-tweaks:wrap-up`'s eager family-gate procedure, and recommends `/claude-tweaks:demo #{n}` on the parent at the same severity and staging. |
