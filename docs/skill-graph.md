...
| `skills/_shared/policy-schema.md` | Owns `integration-branch`, the project-level lever CREATE Step 5.5 reads to decide which branch a routine audits — third in a precedence running `--branch` > `template.branch` > this lever > a branching model documented in CLAUDE.md > git inference. It is the rank that makes an unattended `--defaults` run resolve correctly with no human present. |
