...
| `/init` | Step 14's branch check names `routine.branch` as the recourse when the current branch differs from the GitHub default; Step 15 invokes this skill's `create --defaults --source init` in bulk, which reads that lever. Update Mode's Routine Drift check runs `status --all` then `update --defaults` — the path by which an existing project's live routines pick up a changed template. |
...
| `skills/_shared/policy-schema.md` | Owns `routine.branch`, the project-level lever CREATE Step 5.5 reads to decide which branch a routine audits — third in a precedence running `--branch` > `template.branch` > this lever > a branching model documented in CLAUDE.md > git inference. It is the rank that makes an unattended `--defaults` run resolve correctly with no human present. |
