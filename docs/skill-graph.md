...
| `/demo` | Step 4.8's `acceptance-gap` scope (`_shared/github-pr-scan.md`) finds closed records with no acceptance label via `needsBackstop` (`bin/lib/issues/acceptance.js`) and recommends `/claude-tweaks:demo #{n}` at severity `info` — staged, never auto-applied at any aggressiveness tier, since disposing a closed record is a human judgment call the auto-mode contract keeps off the silenced list. Those records are by construction other sessions' work, so `/demo`'s Step 1 reconstructs their brief from the closing commit (`git log --grep` over `origin/main` + `HEAD`) rather than from session recall, which would have nothing to recall. Step 4.8's `family-gate` scope (same file) is the sibling backstop for decomposition families: it finds parents whose every leaf has closed but which never reached `/claude-tweaks:wrap-up`'s eager family-gate procedure. Its recommended action, `Open family gate` (`tidy/SKILL.md`'s Action Vocabulary), composes and posts the parent's Verification Brief and applies `demo:pending` — reusing `/wrap-up`'s own Family-Gate Procedure — then still recommends `/claude-tweaks:demo #{n}` for the verdict itself. Same `info` severity as `acceptance-gap`, and staged the same way too, at every aggressiveness tier (`tidy/step-6-auto.md`'s Open family gate row is `Stage`/`Stage`/`Stage`) — posting a comment and adding a label is an outward-facing GitHub API write, which fails the auto-mode contract's reversibility floor regardless of how mechanical the write is. `demo:approved`/`demo:changes-requested` stay staged and exclusively `/demo`'s job either way, independent of that write-level reasoning. |
...
| `/tidy` | `/wrap-up` cleans artifacts for a single spec; `/tidy` does periodic bulk cleanup. `/tidy`'s `Open family gate` action (`tidy/actions-github-issues.md`) reuses this file's own Family-Gate Procedure directly rather than restating it — `verification-brief.md` documents two entry *shapes*, leaf-side (any caller arriving mid-close: `/wrap-up` Step 10, `/wrap-up`'s auto-merge short-circuit, `/dispatch`'s group auto-merge gate) and parent-side (`/tidy`, arriving already holding the parent number from its own scan), sharing every step from "Enumerate the family's leaves" onward. |
| `_shared/auto-mode-contract.md` | Single source of truth for auto-mode behavior — read before adding any auto-mode handling. |
