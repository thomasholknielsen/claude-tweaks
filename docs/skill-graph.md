...
| `/demo` | Step 4.8's `acceptance-gap` scope (`_shared/github-pr-scan.md`) finds closed records with no acceptance label via `needsBackstop` (`bin/lib/issues/acceptance.js`) and recommends `/claude-tweaks:demo #{n}` — staged, never auto-applied at any aggressiveness tier, since disposing a closed record is a human judgment call the auto-mode contract keeps off the silenced list. |
