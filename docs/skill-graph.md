...
| `/wrap-up` | `verification-brief.md`'s Step 2.5 safety-net gate invokes `/visual-review` directly (same mode resolution as `/review` Step 6) when an `app-route`/`rendered-page` plan reaches wrap-up without a full pass already having run. Any bug found gates `demo:pending` the same way `/review`'s Step 3 Routing gates PASS. **Not on the family path:** a record with a resolvable parent runs `verification-brief.md`'s Family-Gate Procedure in place of Steps 1-4, and Step 2.5 sits inside that range — see that procedure's "What this path deliberately does not run" for why (no family-scope diff or `/review` summary exists) and what covers the UI instead. |
...
| `/tidy` | `/wrap-up` cleans artifacts for a single spec; `/tidy` does periodic bulk cleanup. `/tidy`'s `Open family gate` action (`tidy/actions-github-issues.md`, or `tidy/actions-local-files.md` on that driver) reuses this file's own Family-Gate Procedure directly rather than restating it — `verification-brief.md` documents two entry *shapes*, leaf-side (any caller arriving mid-close: `/wrap-up`'s Phase 4 execution step, `/wrap-up`'s auto-merge short-circuit, `/dispatch`'s group auto-merge gate, the last passing its whole group as `$CLOSING_LEAVES` so a multi-leaf family gates eagerly instead of falling through to the backstop) and parent-side (`/tidy`, arriving already holding the parent number from its own scan, on either driver), sharing every step from "Enumerate the family's leaves" onward. |
| `_shared/auto-mode-contract.md` | Single source of truth for auto-mode behavior — read before adding any auto-mode handling. |
