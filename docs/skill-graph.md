...
| `_shared/pending-review-durability.md` | `SKILL.md`'s Reporting section cites it directly for what a `pending-review` outcome now leaves behind: the group's run dir still parks, but the branch does not wait with it — whichever console the group reaches runs this procedure first and publishes the branch plus one draft PR. The two consoles (`/flow` and `/wrap-up` rows) invoke and gate it on their own paths; `settle-and-merge.md`'s Dispatching-session merge execution is a third, later-added caller — the Auto-merge gate's own conflict fallback, invoked from dispatch's own thread (not a console) after a direct `git merge` fails post-both-layers-passed. It owns the `OUTCOME` vocabulary that has to stay `pending-review` rather than `pr-opened` once a draft PR exists, which `task-prompt.md`'s reporting template states to the agent. |
...
| `_shared/pending-review-durability.md` | `review-console.md` gates this shared push-and-draft-PR procedure on `CLAIM_RUN_ID`, a worktree strategy, and the resolved step list containing `review`, running it before a single-record run's console renders so a headless `pending-review` outcome's branch survives the container that built it. `verification-brief.md`'s Step 4 renders the outcome it writes as a `### Branch` section. |
