...
| `/tidy` | `/wrap-up` cleans artifacts for a single spec; `/tidy` does periodic bulk cleanup. `/tidy`'s `Open family gate` action (`tidy/actions-github-issues.md`) reuses this file's own Family-Gate Procedure directly rather than restating it — `verification-brief.md` documents two entry points, `/wrap-up` (arrives via a leaf, mid-close) and `/tidy` (arrives already holding the parent number from its own scan), sharing every step from "Enumerate the family's leaves" onward. |
| `_shared/auto-mode-contract.md` | Single source of truth for auto-mode behavior — read before adding any auto-mode handling. |
