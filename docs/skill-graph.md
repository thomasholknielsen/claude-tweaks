...
| Target | Relationship |
|---|---|
| `/dispatch` | The queue consumer — selects records `refine` authorized (`auto:build`), mints the group's run directory (Step 4), and hands each to `/flow`, which claims the group itself at its Step 2.8 (`flow/claim-targets.md`). `/backlog` never claims, dispatches, or executes. `attention` mode reuses `/dispatch`'s own priority-band-then-age ranking (`dispatch/SKILL.md`'s Step 3) for its own list order — not a call, a cited convention. |
| `/specify` | `attention` mode's `needs:definition` rows recommend `/specify #{n}` (the redirect this label triggers there); its `solution:unjustified` rows recommend re-running `/specify #{n}` after adding evidence. Read-only — `attention` never invokes `/specify` itself. |
...
| `/superpowers:brainstorming` | Two triggers. Resolve-the-input case 4: a topic name with no matching design doc invokes it directly, and `/specify` continues with the produced design doc as decomposition-mode input. Resolve-the-input case 1's `needs:definition` redirect: a record reference carrying the label skips shaping mode, invokes it with the record's title + body, and carries `$ORIGIN_RECORD_NUM` forward so decomposition mode's Step 9 can close the origin record once its replacement exists. |
| `_shared/auto-mode-contract.md` | Single source of truth for auto-mode behavior — read before adding any auto-mode handling. |
...
| `_shared/needs-definition-judgment.md` | Not consulted by `/specify` directly — cited here because the `needs:definition` label this redirect keys off is stamped upstream by `/capture`/`/feedback` against this shared rubric. |
