...
| Target | Relationship |
|---|---|
| `/backlog` | `refine` calls `grant-check` once per worklist record; the output becomes the unified table's Recommended column for grant rows directly. `refine` still renders the human batch-confirm exactly as before — only what generates the suggestion changed. `grant` mode (the headless machine-grant unit, `grant-mode.md`) calls `grant-check` too, as gate 4 of `bin/lib/issues/grant-gate.js`'s chain — same call shape, but here the returned `RECOMMEND_BUILD` directly gates a machine-written label instead of feeding a human-confirmed table row. |
...
| Target | Relationship |
|---|---|
| `/dispatch` | The queue consumer — selects records `refine` authorized (`auto:build`), mints the group's run directory (Step 4), and hands each to `/flow`, which claims the group itself at its Step 2.8 (`flow/claim-targets.md`). `/backlog` never claims, dispatches, or executes. |
...
| `/review` | Invokes `review` mode during code review (Step 6.5). Findings appear as a "Design Quality" section in the review summary — advisory, not blocking. `review` mode also writes an audit cache (`docs/plans/...-audit.json`) consumed by `polish`. When the changed artifact carries a direction contract, the findings list additionally carries `source: "finish-review"` rows adapted from upstream's agent (Step 3.7), alongside a `finish_review.keep` line the summary renders above them. |
| `/tidy` | Invokes `doctor` mode as Step 4.9's scan step, passing `--source tidy` unconditionally (`/tidy` is standalone-only and never has a `$PIPELINE_RUN_DIR` to forward). `doctor` delegates wholesale to the pinned plugin's `doctor.mjs --json` — never `--fix` — and returns drift findings about the project's own Impeccable artifacts. `/tidy` renders them in a Design Record Drift section, deliberately not the Actions table: the rows are surface-or-suppress and this path mutates no project file at any aggressiveness tier. On a skip it degrades silently, with no row at all. |
...
| `_shared/design-craft.md` | `modes/pre-build.md` is the contract's main implementation: it resolves and loads Emil's skills per the contract's resolution procedure and relevance map, loads the `.impeccable/design.json` sidecar alongside `DESIGN.md`, and appends the contract file itself to `loaded` so implementers receive the authority rule verbatim. `modes/explore.md`'s two renderer dispatch sites are also wired consumers: principles-only at identity scope (genesis — no decisions exist), decisions + principles at layout scope. |
...
| `_shared/pr-first-merge.md` | `settle-and-merge.md`'s Auto-merge gate runs this procedure directly inside the second Task call — no more cwd-pinned/main-checkout thread split, no `OUTCOME: ready-to-merge` relay, on the `pr-first` path (`local-merge` keeps the old two-thread shape as a retained fallback). Supersedes and closes #335 and #299. Shared with `/wrap-up` (`review-console.md`'s fast-lane short-circuit) and `/flow` (`worktree-merge.md`'s multi-branch reconciliation) — owned here as the alphabetically-first of the three citing skills. |
