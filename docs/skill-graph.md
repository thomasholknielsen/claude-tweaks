...
| `_shared/design-craft.md` | `modes/pre-build.md` is the contract's main implementation: it resolves and loads Emil's skills per the contract's resolution procedure and relevance map, loads the `.impeccable/design.json` sidecar alongside `DESIGN.md`, and appends the contract file itself to `loaded` so implementers receive the authority rule verbatim. The `explore` mode's renderers are an intended additional consumer (#386, not yet wired). |
