...
| Fleet scheduling (`/claude-tweaks:routine`) | Not yet wired — the companion leaf blocked on this one ships `backlog/routine-template.yml` and the eval scenarios that schedule `backlog grant` as a recurring headless Routine, the same way `dispatch/routine-template.yml` schedules `dispatch next` today. |
