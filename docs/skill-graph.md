...
| Target | Relationship |
|---|---|
| `/dispatch` | The queue consumer — claims records `refine` authorized (`auto:build`) and hands each to `/flow`. `/backlog` never claims, dispatches, or executes. `attention` mode reuses `/dispatch`'s own priority-band-then-age ranking (`dispatch/SKILL.md`'s Step 3) for its own list order — not a call, a cited convention. |
