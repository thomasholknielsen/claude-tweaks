...
## IL-70 — Inlining into a size-capped file, then extracting
...
## IL-71 — Planning work from a filed issue's stale measured evidence

Don't plan work from an audit-filed issue's "Measured:" evidence without re-verifying it against live code — a filed issue is a snapshot, and the fix it asks for may already have shipped.

Issue #92 asked to exclude `docs/superpowers/plans/**` and `docs/superpowers/specs/**` from `/docs-health`'s rotation, citing a measured table of "the three largest files currently in scope," all under the archive. That exclusion had shipped in `8d7d3419` on 2026-07-17; the issue was filed 2026-08-02, two weeks later. `bin/lib/docs-health/scope.js`'s `EXCLUDE_TOP_LEVEL_DIRS` already covered it, already matched on the full path (so the near-identically-named `docs/plans/` correctly stayed in scope), was already documented in the skill, and was already tested. Running `listDocs()` took seconds and showed 15 targets, 0 under `superpowers`, largest 28,351 B — none of the issue's three cited files. Only the byte cap remained genuinely unbuilt. The issue body reads as authoritative measured evidence, which is exactly why it invites being trusted; the acceptance criteria were checked but the premise was not.
