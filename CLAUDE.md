...
- Don't attach extra properties to an array before `JSON.stringify`-ing it (e.g. in `node -e ... console.log(JSON.stringify(...))` shell wiring) — non-index properties are silently dropped with no error; return a plain object with the array as a named field instead `[IL-121]`
- Don't use an arbitrary short placeholder (e.g. `'aaaa'`) as a fake ID/SHA/token in a regression test when the code path under test enforces a length or format constraint on that value — the fake input can fail to reach the exercised branch entirely, so the test passes or fails for the wrong reason. Match the target's real shape (e.g. 40-char hex for a git SHA) `[IL-122]`
