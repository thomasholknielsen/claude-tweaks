...
- Don't treat a check's green as evidence before naming what its red would look like — a count-based grep, a suite that never loaded the file, and a sweep that silently dropped part of its input all pass identically whether or not the thing they check is true `[IL-104]`
