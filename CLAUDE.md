...
- Don't put detailed reference content inline in a SKILL.md — extract to a sub-file, cited as "read `{filename}` in this skill's directory". **40 KB is a soft ceiling per SKILL.md and per sub-file**: `Read` has no section granularity, so once two stubs cite *sections* of one sub-file, every stub pays for the whole file. Split by the stubs' own unit, extract rather than reorganize in place, leave the original heading as a stub so external references still resolve, and confirm every substantive line survives somewhere — no test reads skill prose `[IL-70]`
...
- Don't resolve a conflict against a *structural* refactor by picking a side — content in the old location must be re-homed. Upstream refactored: take its structure, verify byte-identity, re-apply your additions. *Your* branch refactored, upstream edited the old file in place: mirror applies — keep your structure, re-home upstream's edits into the new files `[IL-44]`
...
- Don't trust a "render this, then call the tool" instruction to bind at runtime — nothing stops the tool firing from a response that never rendered, and an approval never implies a differently-scoped write is authorized. State an explicit pre-call check for the first, a dedicated per-decision approval for the second — `[IL-102]`'s hazard at the interaction boundary `[IL-114]`
...
- Don't call `ExitWorktree`/`git worktree remove` directly to finish a pipeline-run worktree, even after verifying no commits lost — it skips `cleanup-procedures.md` Section C step 3.5's Transitional guard, permanently destroying the run's gitignored `config.yml`/`decisions.md`/`staged/` with no git history to recover. Route worktree teardown through wrap-up's own Step 10 cleanup execution instead `[IL-116]`
