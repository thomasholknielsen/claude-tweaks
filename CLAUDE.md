...
Forms B and C always pair with the **Subagent Contract** (`skills/_shared/subagent-output-contract.md`) — minimal input, one of `DONE / DONE_WITH_CONCERNS / NEEDS_CONTEXT / BLOCKED` as the agent's first line, then Templates A/B/C for output. Each dispatch picks a work profile (`Fast | Standard | Capable`; `Frontier` only at contract-enumerated singleton slots, never in a fan-out) resolved via `bin/resolve-profile.js`; default to the cheapest that fits the work. Third-party agents are exempt from the agent-side protocol (see the Subagent Contract section below); the dispatcher's side still applies in full.
...
- **Setup script (required, not optional):** paste `bash scripts/claude-cloud-setup.sh` into this project's cloud environment's Setup script field (claude.ai/code environment settings, web UI only — no API/CLI can set this remotely). Installs every declared plugin/marketplace plus `agent-browser`. Regenerated by `/claude-tweaks:init` Step 14; don't hand-edit it. Without it, a declared plugin is simply absent. This applies per *environment*, not per repo — an environment selected in the session composer that has never had this pasted fails this way even for a fully-declared repo `[IL-113]`.
...
- Don't read a committed artifact from the working checkout without first establishing the checkout is current — `git status` and `git branch -vv` both measure against the stale remote-tracking ref, so a checkout 119 commits behind looks clean; fetch first or read from the ref, and gate any resulting stop on a *verified* comparison so offline degrades instead of blocking `[IL-104]`
...
- Don't treat a check's green as evidence before naming what its red would look like — a count-based grep, a suite that never loaded the file, and a sweep that silently dropped part of its input all pass identically whether or not the thing they check is true `[IL-105]`
...
- Don't start a record after checking only branches, claims, and labels — a live session's unpushed work is invisible to all three. Enumerate `git worktree list --porcelain` and `ps` its lock-owning pids; a finished nine-task branch was nearly redone `[IL-107]`
- Don't verify a content assertion by checking it matches the prose you just wrote — that is guaranteed. Negate the prose and assert the regex *fails*, one claim at a time (a multi-assertion test short-circuits); a bare token or a wide `[\s\S]{0,N}` window routinely survives inversion, and 11 did in one run `[IL-105]`
- Don't put a long-running command between an implementer subagent's last edit and its commit — 4 of 4 stalled at exactly that wait with work uncommitted, and an explicit "don't park" instruction didn't help. Commit after focused tests; run the full suite centrally `[IL-108]`
- Don't close a hazard you noticed with a mitigation you did not execute — a remedy can create a worse instance of the same hazard, and "resolved" is what stops anyone re-deriving it. Mandating the canonical integration-branch ladder (which yields bare names) blinded the one consumer needing a remote-tracking ref, on the repo that wrote the rule `[IL-111]`
- Don't believe a check's red before checking the check — it asserts the code is broken *and* the harness sound. Suspect the harness first when the change altered how many calls the code makes, or in what order; `[IL-105]` is this pointed at green, and green is the half people remember `[IL-112]`
