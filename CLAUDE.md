...
- Don't write an input-shape guard checking only `typeof`/`null` — verify the specific fields the consumer actually reads. A shallow object-check still admits a same-`typeof` value (an array) or a same-shape-but-missing-keys value that crashes or silently no-ops downstream `[IL-120]`
