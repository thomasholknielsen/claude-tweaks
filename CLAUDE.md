...
- Don't run a health CLI (`bin/{code,harness,journey,docs}-health.js`) with real args to test a change — it pushes durable state to the shared `health-state` branch and stamps a rotation cursor suppressing that target for 90 days. Exercise the module or its unit suite instead `[IL-73]`
