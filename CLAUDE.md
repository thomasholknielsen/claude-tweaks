...
- **Surgical changes.** Touch only what the task requires. Don't reformat or "improve" adjacent code. Match the surrounding style. A pre-existing bug or unrelated concern you notice is a follow-up to report, not a fix to fold in, unless the requested behavior cannot work without it. Edit a file in place rather than rewriting it when the result is the same — a rewrite costs more and hides what changed.
