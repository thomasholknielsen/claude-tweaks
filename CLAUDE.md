...
- Don't rely solely on a migration plan's literal-path grep to confirm a terminology retirement — it can't catch generic-vocabulary occurrences of the same concept. Bake a bare-word sweep into every dispatch from task one `[IL-21]`
...
- Don't trust a markdown edit that inserts prose next to a fenced code block by reading the result — verify the fence still closes correctly; a misplaced sentence lands *inside* it and breaks the snippet when run `[IL-27]`
...
- Don't resolve a conflict against an upstream *structural* refactor by picking a side — content still in the old location must be re-homed. Take upstream's structure wholesale, verify byte-identity, then re-apply your additions `[IL-44]`
...
- Don't inline a large block into a size-capped file and plan to extract later — when a fix's deliverable is "inline N KB into a prompt," check the host file's budget first and extract to a sub-file the caller inlines *from* `[IL-70]`
- Don't plan work from an audit-filed issue's "Measured:" evidence without re-verifying it against live code — a filed issue is a snapshot, and the fix it asks for may already have shipped `[IL-71]`
