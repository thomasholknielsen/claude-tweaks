...
- Don't cite a figure from a plan's own measurement command without checking its boundary behavior — a design doc generated by the same command corroborates nothing `[IL-78]`
- Don't edit a reference in a dispatcher-inlined file without confirming which region actually gets inlined — a test suite's own split point tells you `[IL-79]`
- Don't place a special-case exemption after an early return that can claim the same value — the exemption only runs on whichever branch you didn't put it after `[IL-80]`
- Don't add a `bin/lib/{name}/tests/` directory without adding its glob to `package.json`'s test script — an enumerated list of globs doesn't pick up a new one on its own `[IL-81]`
