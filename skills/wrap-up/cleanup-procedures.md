...
|---|---------|---------------|-----------|--------------------------------------------|
| 1 | Execution plans | Delete ephemeral plan files in `~/.claude/plans/` related to this spec (Claude Code's own native plan-mode scratch output). **Do NOT delete `docs/superpowers/plans/*.md`** — this project's CLAUDE.md documents it as a permanent historical archive, pruned only as a separate, deliberate maintenance action, never per-build. This applies regardless of build mode (design-mode or otherwise) — a spec-specific plan file there is kept, same as every other. (Design docs `*-design.md` in `docs/superpowers/specs/` should already be gone — `/specify` deletes them. If any remain, delete now.) | spec-based work | No (idempotent — leave to per-spec wrap-up) |
