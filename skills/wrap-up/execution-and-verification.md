...
- **Memory updates** — already written when approved. The console executes: for each approved `M#` item, `review-console.md`'s "On approval" step 8 (auto/hybrid mode) — or `wrap-up/SKILL.md` Step 7.10's per-item batch-decision equivalent (interactive mode, standalone wrap-up) — wrote the memory file and appended its `MEMORY.md` index line per `_shared/learning-routing.md`'s "Memory write procedure (D4)" at the moment of approval. Step 10 does not write it again; Step 10 confirms it landed — see Verify execution below
- **Upstream feedback** — already filed when approved. The console executes: for each approved `U#` item, `review-console.md`'s "On approval" step 9 (auto/hybrid mode) — or `wrap-up/SKILL.md` Step 7.11's per-item batch-decision equivalent (interactive mode, standalone wrap-up) — invoked `/claude-tweaks:feedback` with the staged, already-scrubbed body at the moment of approval. Step 10 does not invoke it again; Step 10 confirms the filing landed — see Verify execution below
