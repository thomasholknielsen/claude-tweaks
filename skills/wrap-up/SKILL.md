...
| `/claude-tweaks:assess-agent-autonomy` | Step 8.6's Auto-merge short-circuit (`review-console.md`) calls `merge-check` directly — the single-record version of `/claude-tweaks:dispatch`'s own group-scoped Auto-merge gate, run whether or not `/claude-tweaks:dispatch` was involved. A `needs-human` verdict, or either layer failing, falls through to rendering the console normally. |
