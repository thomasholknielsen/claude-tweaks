...
| Worktree reap (`bin/hooks.js session-start`, every `SessionStart`) | N/A — fires before any conversation exists, so there is no interactive equivalent to silence | Always resolves at the **log** tier (CLAUDE.md Hooks: block/warn/inform/log): appends an `events.jsonl` entry per worktree reaped or skipped and reports the result in the SessionStart banner. Not user-facing decision-worthy — there is no `AUTO`/`STAGED`/`KEPT-PROMPT` routing, since the five-part safety predicate (Tasks 5-6 of the worktree-reaping design) is the only gate, not a reversibility/confidence/severity floor. |
