...
| Wrap-up's per-spec Review Consoles under multi-spec `/flow` | One console per spec at each wrap-up (N consoles for N specs) | **Consolidated into one** end-of-run Review Console at `/flow`. Per-spec wrap-ups skip their own console when `MULTISPEC_REVIEW_DEFER=1` is set; the parent `/flow` runs a single console reading the parent run dir's `decisions.md` + `staged/` and every per-spec `decisions.md` + `staged/` after the last spec completes. Preserves the bookend promise (one start stop, one end stop) regardless of N. See `flow/multispec-review-console.md`. |
