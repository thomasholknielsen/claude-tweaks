...
The second (#537): an `Edit`/`Write`/`NotebookEdit` whose target, once fully resolved to a real path (symlinks followed, `..` normalized, on-disk casing canonicalized), IS the repo root's `.claude-tweaks/policy.yml` — exact identity, never containment, so a symlinked alias resolves to the same allow and `policy.yml` itself being swapped for a symlink elsewhere resolves to the same deny. Alongside it, `git commit` is allowed when the **entire command string** matches an allowlist grammar — exactly `git commit` plus one or more `-m`/`--message` args and an optional `--no-verify`, in any order, and nothing else: no other flag, no pathspec, no shell operator (`&&`, `;`, `|`, `` $() ``, backticks), no env-var prefix, no path to `git` other than the bare word — **and** the staged set (`git diff --cached --name-only`) is provably nothing but `.claude-tweaks/policy.yml`. `git push` stays gated regardless. Both exemptions fail closed: anything unprovable about a path, a command's grammar, or the staged set keeps the deny.
