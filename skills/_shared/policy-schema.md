...
**Derived-default keys** come in two shapes. Both are live; neither is the precedent by default, so a new derived key picks one deliberately rather than rediscovering the pieces.

*Shape A — resolve-time derivation, no static default* (`integration-model`, `merge-verification`): the `POLICY_KEYS` row carries no static `default` (a literal would bypass the derivation); the derivation's single prose statement lives in this file (`_shared/integration-model.md`'s ladder via forge detection; the `merge-verification` coverage block below) with a code twin in `bin/lib/` (`policy-schema.js`'s `detectIntegrationModel`; `merge-verification.js`'s `deriveMergeVerification`); `bin/resolve-policy.js` computes the value only for an *absent* key (`source: "default"`, no `invalid` flag) and never overwrites an `invalid: true` envelope; and `/claude-tweaks:help`'s policy mode renders the default as `computed (…)` rather than `no default`.

*Shape B — in-loop derivation over a static base default* (`housekeeping-auto-merge`, #580): the `POLICY_KEYS` row keeps a static `default` as the base value (`false` — the `supervised` base), so the rendered schema default and the effective default differ; the derivation runs inside `bin/lib/policy-schema.js`'s `resolvePolicyKeys` (`deriveHousekeepingAutoMerge`), not in `bin/resolve-policy.js`; it fires for any entry that already resolved `source: "default"`, which deliberately covers **both** unset and set-but-invalid — an `invalid: true` envelope keeps its flag but takes the derived value; and the entry keeps `source: "default"` precisely because that field is the derived-vs-explicit attribution surface its consumers read. Shape B is the choice to make when an existing consumer must keep reading the value as a plain default; Shape A otherwise. `/claude-tweaks:help`'s policy mode does **not** yet render Shape B's default as `computed (…)`.

Which shape a further derived key should take is not settled beyond that rule of thumb — #580's ledger records the coexisting implementations and defers consolidation until another such lever appears. State the chosen shape and its reason in the new key's row. Adding a lever of either shape also walks `_shared/auto-mode-contract.md`'s "Adding a new policy lever" checklist when it is Manifesto-visible.
