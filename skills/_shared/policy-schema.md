...
These levers currently have no documented `policy.yml` path at all — CLAUDE.md is their only home today. Three of them (`section-confirmation`, `scope-keywords-required`, `merge-check`) are still actively written into every new CLAUDE.md by `/claude-tweaks:init`'s template today — they were never part of the retired "Auto-mode policy" block this spec addresses, so their generation is unaffected by this spec; only their presence in this schema is new. `backlog-fetch-limit` and `promise-register-min-leaves` also appear in `_shared/work-record.md`'s Config keys table — if the two disagree, that file wins for those two keys, per the same rule the "Dispatch & merge" section states.
...
| `scope-keywords-required` | CLAUDE.md only — no `policy.yml` path documented today | `/claude-tweaks:build` | `false` | When `true`, `/build`'s plan-audit Check B refuses to start if any matched files aren't in the plan AND the plan/design has no `Scope keywords:` field — otherwise (default `false`) this is informational only, a warning |
| `section-confirmation` | CLAUDE.md only — no `policy.yml` path documented today | `/superpowers:brainstorming`, `/claude-tweaks:deepen` | `adaptive` | Whether a skill's multi-section approval gate batches after 2 clean approvals (`adaptive`), always asks per-section, or always batches once |
| `merge-check` | CLAUDE.md only — no `policy.yml` path documented today | `/claude-tweaks:build`, `/claude-tweaks:flow` | `true` | Whether `/build`'s and `/flow`'s pre-flight step compares the current branch against its upstream and offers rebase-vs-continue; `false` skips this check |
