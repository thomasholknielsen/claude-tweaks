...
| `/claude-tweaks:tidy` (`SKILL.md` Step 7's housekeeping-marker note) | Whether Step 7's commit is ever eligible to carry the `<!-- tidy-housekeeping-pr -->` marker at all — a PR-only concept, so the note applies only under `pr-first` |
...
| `/claude-tweaks:wrap-up` (`review-console.md`'s Console-on-PR section), `/claude-tweaks:flow` (`multispec-review-console.md`'s Console-on-PR section) | Routing the top-level Review Console gate to `_shared/console-on-pr.md`'s PR-checkbox render (`pr-first`) vs. the unchanged blocking `AskUserQuestion` (`local-merge`) |
