...
`code-health`, `harness-health`, `journey-health`, and `docs-health` each run two unconditional mechanics inside their own FILE step, before the interactive ask-before-file gate (`_shared/health-filing-gate.md`) ever runs: draining the durable retry queue from a prior firing's filing failures, and reopening an issue whose finding has regressed. This file is the one place the *shape* of both mechanics is defined, so a correctness fix made to one skill's copy can be checked against the same canonical shape in the other three. Per this project's own convention that a skill file must be self-contained for whichever session reads it (a dispatched agent cannot "go read" a sibling skill's file mid-run), each consumer still writes out the actual bash commands in full inline — this file is the reference the four copies are kept in sync against, not a replacement for any of them.
...
## Both mechanics execute unconditionally

Neither mechanic is gated by the interactive ask-before-file decision (`_shared/health-filing-gate.md`'s own Scope section states this explicitly) — a retry-queue drain was already approved in the firing that first produced it, and reopening isn't creating anything new. Both happen before the gate ever runs, and before `--dry-run` mode suppresses any `gh` call.
