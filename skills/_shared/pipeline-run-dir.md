...
**Record-mode materialization exception.** `/claude-tweaks:build #{n}` invoked standalone (no `/flow` parent) creates its own standalone run dir — `.claude-tweaks/pipelines/{ISO-timestamp}-record-{n}-standalone/` — via `skills/flow/materialize.md`'s own fallback when neither step 1 nor step 2 above resolves, purely as artifact storage for the materialized file. This is distinct from step 3's auto-decision allowlist: it fires regardless of mode (`/build` is not itself on that allowlist), since materialization needs somewhere to write the file whether or not `auto` is active.
