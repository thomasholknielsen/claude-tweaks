...
3. **Standalone auto fallback** — when neither resolves AND the skill is running in `auto` mode AND the skill is on the standalone-auto allowlist (`/tidy`, `/init`, `/capture`, `/reflect`, `/journeys`, `/visual-review`, `/simplify`, `/claude-tweaks:triage`), create a standalone run dir at `.claude-tweaks/pipelines/{ISO-timestamp}-{skill-name}-standalone/` with `decisions.md` and `staged/`. The audit log stays on; the skill auto-resolves per project policy in CLAUDE.md. The dir is presented in a Pending Review section at the end of the skill's report (no separate Review Console — this is the bookend-end for a standalone run).

   `/claude-tweaks:triage`'s `dispatch` mode is a special case: it runs unattended (headless, no human present), never in bare `interactive` mode, so step 4's interactive fallback below is not a real option for it — `dispatch` always needs a standalone run dir to resolve, which is why it's on this allowlist despite not being one of the original "auto-mode skills."
