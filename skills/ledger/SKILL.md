...
| Using the ledger for feature tracking | The ledger tracks findings and tasks within a single pipeline run. Cross-spec/cross-run tracking belongs on the decomposition's parent record's `## Cross-Spec Promises` section instead (`_shared/work-record.md`). |
