...
- Option 3 — `label`: `"Shape the top priority record"`, `description`: `"/claude-tweaks:specify #{n} — shape the single highest-priority backlog record this run surfaced"`
...
If situational filtering leaves only one option (a bare run whose Dispatch block is empty, that surfaced nothing needing refinement, and is this session's first lens run leaves Option 3 alone), state or execute it directly instead of calling `AskUserQuestion` — per this project's own convention, a lone option isn't a decision. The same rule applies to the `refine` block above.
