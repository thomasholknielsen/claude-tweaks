...
| **Family-gate** (decomposition family complete with no acceptance disposition on the parent, surfaced by `github-pr-scan.md`'s `family-gate` scope; no mutation, recommends `/claude-tweaks:demo #{n}`) | Auto (no-op, always surfaced) | Auto (no-op, always surfaced) | Auto (no-op, always surfaced) — never auto-applied at any tier, same reason as Acceptance-gap: disposing a record is a judgment call the auto-mode contract keeps off the silenced list (`_shared/auto-mode-contract.md`) |
