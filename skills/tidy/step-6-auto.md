...
| **Open family gate** (decomposition family complete with no acceptance disposition on the parent, surfaced by `github-pr-scan.md`'s `family-gate` scope; composes and posts the parent's Verification Brief, applies `demo:pending` — `tidy/actions-github-issues.md`'s `## Open family gate`, reusing `wrap-up/verification-brief.md`'s Family-Gate Procedure) | Auto-apply | Auto-apply | Auto-apply — opening the gate is not the disposition. `demo:approved`/`demo:changes-requested` are the judgment call the auto-mode contract keeps off the silenced list, and this action never applies either one — that stays exclusively `/claude-tweaks:demo`'s job, always staged and human-only. Opening the gate is a mechanical, fully reversible precondition (add a label, post a comment), and `/claude-tweaks:wrap-up` already performs the identical write with zero staging whenever its own eager path fires — this backstop mirrors that rather than adding ceremony the primary path never had. This is a deliberate exception to the "outward-facing GitHub mutation → always Stage" pattern the rows below follow (Close (GitHub) / Resolve thread, `github-issues` Defer/Sync/Delete/Absorb): those rows are staged because of collaborator visibility and notifications, a real concern this row shares, but the auto-mode contract's actual floor here is the disposition, not visibility — see `_shared/github-pr-scan.md`'s `family-gate` scope for the fuller reasoning. Before applying, this action re-verifies the gate is still `due` with freshly fetched state — never trusts the scan's own snapshot, which may be stale by the time Step 7 runs. |
