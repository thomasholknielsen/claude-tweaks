...
| **Mark as specified** | A `docs/superpowers/specs/*-design.md` doc carries no status marker yet, and its scope already matches existing specs — stamp it so a later sweep can retire the doc (Step 3's design-doc classification) | Insert `Status: specified — decomposed to {record refs}` directly below the doc's title heading (`# {title}`) — see `scan-procedures.md` Step 3 | No — doc stays, stamped in place |
...
- [x] Marked as specified: "{title}" — `docs/superpowers/specs/{filename}` re-read and found to carry `Status: specified — decomposed to {record refs}` directly below its title heading (`scan-procedures.md` Step 3)
