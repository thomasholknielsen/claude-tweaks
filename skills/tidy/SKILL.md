...
Issue those Step 4/4.6/4.9 tool calls in the same message that dispatches the agent batch above — they depend on neither it nor each other, so the whole batch overlaps. Being in the main thread they need no status line and no agent envelope, and the Contract/Model tier/Output template above do not apply to them; their findings feed the Step 6 report directly in the same `[plan]`/`[registry]`/`[doctor]` format, read through the same column semantics. Keep Step 4's judgment metadata-only (filename, file age, whether a related record exists and is still open) — do not read plan bodies into the main thread, which is the cost this demotion exists to avoid. All three steps stay `--scope`-selectable exactly as before (`plans` → 4, `registry` → 4.6, `design` → 4.9); only the execution mechanism changes, not which steps a scope covers.
...
**Under `worktree.always: true`:** `/claude-tweaks:tidy` is standalone-only and never creates or enters a worktree via its own steps (see Component-Skill Contract below). If the target project has `worktree.always: true` set (`.claude-tweaks/policy.yml`), every mutation in Step 7 (record file deletes/edits, `docs/REGISTRY.md` Fix-now edits) and this commit are `Edit`/`Write`/`git commit` calls the PreToolUse gate denies outside a linked worktree — there is no standalone-auto exemption for these (`_shared/auto-decision-log.md`'s Bash-append workaround covers only the `decisions.md` audit-log write, not Step 7's substantive edits or this commit). Before executing Step 7, resolve the lever via the canonical read path — `node "${CLAUDE_PLUGIN_ROOT}/bin/resolve-policy.js" --values worktree.always` — and if it resolves `true`, provision a scratch worktree, run Steps 7 and 7.5 inside it, then return to the main checkout — following `_shared/scratch-worktree.md`'s full procedure end to end (creation, applying the commit, returning to the integration branch, teardown) rather than a restatement here. That file's §6 is the fix for what this step used to say: tear down via `ExitWorktree`, never a raw `git worktree remove` — the scratch worktree is by definition the one this session just worked inside, so the harness holds a live lock and the raw command exits 128 (`[IL-58]`; this is exactly why "mirroring Step 4.5's own worktree cleanup" was wrong here — Step 4.5 removes worktrees it is *not* standing in). Skip this entirely when `worktree.always` isn't set, or when `--dry-run` was passed (Step 7 never mutates or commits in that case).
...
**The `housekeeping-auto-merge` grant and the `<!-- tidy-housekeeping-pr -->` marker** (`_shared/policy-schema.md`, `_shared/github-pr-scan.md`'s `repo-wide` item 9): when this run's commit is ever pushed as a PR under `integration-model: pr-first` (`_shared/integration-model.md`), stamp the PR body with `<!-- tidy-housekeeping-pr -->` at creation — that marker, not a label, is what lets the sweep identify a tidy-originated PR without a local run-dir join. With `housekeeping-auto-merge` set project-wide, a green, marker-stamped PR may arm `--auto` at `moderate`+ aggressiveness with no further grant (`tidy/actions-github-issues.md`'s `## Arm ready PR`); unset, it stages like any other unarmed PR. **As of this writing, Step 7 above does not itself open a PR** — the `worktree.always` path merges back directly (`_shared/scratch-worktree.md`), and the no-worktree path commits straight to the current branch — so this marker has nothing to stamp yet under either path; a queue-write proposal tracking that gap is staged for this run's Review Console.
