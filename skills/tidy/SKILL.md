...
- Up to 3 more options (when Yours items exist and Apply-all-staged is absent) or up to 2 more options (when Yours items exist and Apply-all-staged is present) — one per Yours item as derived above, first option overall suffixed `(Recommended)` only when the "Apply all staged" option is absent
