...
- **The two acceptance backstops — `[family-gate]` and `[acceptance-gap]` — sit in a different scope on each driver.** Both come from Step 4.8 (`github`) under `work-backend: github-issues` and from Step 1 (`backlog`, and `specs` through it) under `work-backend: local-files`, since the local sweeps read the record store rather than the GitHub API. `--scope=github` surfaces neither on the local driver, and `--scope=backlog` surfaces neither on the GitHub one; an unscoped run covers both regardless.
...
|------|-------------|--------------|
| 1 (rules in `step-1-records.md`) | Open work records — `gh issue list` facet-parsed by `record.js`'s `parseRecordFacets` (`github-issues`), or `local-store.js`'s `queryRecords('specs', {})` (`local-files`); those two plus `writeRecord` are the whole record driver in `bin/lib/issues/{record,local-store}.js`. Under `local-files` only, Shapes 7 and 8 each run their own `queryRecords` pass, since the shared fetch above returns open records only: Shape 7 pairs an open-parent query (`{ familyParent: true }`) with an open+closed leaf merge per family, and Shape 8 queries closed records directly (`{ closed: true }`) and keeps every one that is not a decomposed leaf — a closed parent surfaces there, only leaves are suppressed | `[backlog]` / `[parked]` / `[unsynced]` / `[scoring]` / `[blocked]` / `[legacy]` / `[family-gate]` / `[acceptance-gap]` (those last two `local-files` only — Step 4.8 emits both on the other driver) |
