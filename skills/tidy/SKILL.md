...
|------|-------------|--------------|
| 1 (rules in `step-1-records.md`) | Open work records — `gh issue list` facet-parsed by `record.js`'s `parseRecordFacets` (`github-issues`), or `local-store.js`'s `queryRecords('specs', {})` (`local-files`); those two plus `writeRecord` are the whole record driver in `bin/lib/issues/{record,local-store}.js`. Under `local-files` only, Shape 7 adds its own open+closed `queryRecords` pass over decomposition families | `[backlog]` / `[parked]` / `[unsynced]` / `[scoring]` / `[blocked]` / `[legacy]` / `[family-gate]` (`local-files` only — Step 4.8 emits it on the other driver) |
