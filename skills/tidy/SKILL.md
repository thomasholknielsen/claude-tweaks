...
| Escalating `git branch -d` to `git branch -D` when delete refuses | `-d` refusing means only "not contained in HEAD/upstream" — not unmerged work, since a branch merged into a *different* configured base refuses identically. Check the branch against every configured base before concluding; surface `merged into {other-base} — needs -D, manual review required` vs. genuinely `unmerged — manual review required`. Never destructive-delete autonomously either way. |
