...
| `/claude-tweaks:wrap-up` | Hard-to-reverse interface trade-offs /deepen flags `[ADR-candidate]` in Step 4 are picked up by /wrap-up's Step 6.2 and run through the 3-factor gate for possible ADR creation — see the `_shared/decision-records.md` row below. |
