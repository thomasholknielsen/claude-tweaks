...
**Genesis-moment recommendation.** Option 1's own `document` run above already writes `DESIGN.md`, so this applies whenever the step ends `enabled` or `plugin-only` on a web-frontend project with no `DESIGN.md` yet at the project root — option 2 ("Plugin only") skipped the interview entirely, or option 1's interview didn't produce one. When that's the case, the step's summary/next-actions output recommends `/claude-tweaks:design-wrapper explore` as the genesis move: it compares competing visual identities in the browser and, on a pick, locks `DESIGN.md` via upstream. This is recommendation text only, never a `Skill` tool invocation — `explore` needs a human present in a browser to judge the tournament, and `/init` may run in cloud or Routine contexts where none exists.
