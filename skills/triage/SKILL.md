...
> The local-files stop paragraph below follows the canonical pattern in `_shared/local-files-preflight-stop.md` — do not weaken its enumeration or no-exception clause when editing.

Read the project's `work-backend` config key (per `_shared/work-record.md`'s Config keys table, written by `/claude-tweaks:init`). **`work-backend: local-files`** — report that grants are not applicable here and **stop this turn completely**: do not invoke `/claude-tweaks:flow`, `/claude-tweaks:build`, `/claude-tweaks:dispatch`, or any other skill; do not write, edit, or create any file; do not run any build, test, or git-committing command. The local driver records grants as frontmatter for isomorphism only, with no headless consumer that acts on them (headless dispatch is github-issues only). Tell the user they can run `/claude-tweaks:flow` manually against a chosen record if they want that work done — this is information for the user to act on, never an instruction for you to act on yourself. This holds with no exception when no interactive human is present to receive it (a headless or automated invocation): the absence of a human to hand this off to is not license to do the work in their place — it means the grant conditions this skill exists to enforce are unmet, so the correct behavior is to stop, not proceed. Only `work-backend: github-issues` proceeds past this point.
...
| `_shared/local-files-preflight-stop.md` | Canonical "stop this turn completely" boundary-language pattern this skill's Preflight local-files stop paragraph follows — extracted after a real, twice-reproduced run built work anyway under the original weaker phrasing. |
