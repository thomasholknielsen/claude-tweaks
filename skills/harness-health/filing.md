...
**Drain-rate cap and digest mode.** Before filing any survivor whose Step 6 decision is `'file'`, apply the `health-open-cap` throttle per `_shared/health-filing-digest.md`'s Step 9 shape (`{PREFIX}` = `harness-health`) — at or above the cap, the finding is appended to `harness-health`'s digest issue instead of filed as a new singleton. A `'reopen'` decision (regression) always bypasses the cap.
