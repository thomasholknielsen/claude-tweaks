...
| `/claude-tweaks:journey-health` | Sibling health skill for `docs/journeys/*.md` accuracy and agent-e2e coverage — same SELECT → JUDGE → FINGERPRINT/DEDUP → FILE pipeline (journey-health has no verify-gate step, unlike this skill's own Step 5) and `_shared/health-state.md`'s durable persistence, scoped to journeys instead of skill/rule/CLAUDE.md accuracy. |
