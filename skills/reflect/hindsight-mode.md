...
If any findings are **"Change now"**, apply the ones whose destination permits direct application; for the rest, write the `[route: D4]`/`[route: D5]` tagged ledger entry above so `/claude-tweaks:wrap-up` Step 7.1 stages it via Step 7.10/7.11, then re-run `/claude-tweaks:test` (or verification if standalone) and resume.
