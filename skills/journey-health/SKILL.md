...
|---------|--------------|
| Editing a journey file, story YAML, or code directly | `/journey-health` only ever judges and files — every fix routes through `/claude-tweaks:journeys` or `/claude-tweaks:stories`, invoked by a human directly, or by `/claude-tweaks:dispatch` → `/claude-tweaks:flow` once `/claude-tweaks:backlog refine` has granted `auto:build`. |
