...
| Running the deep tier's dev server without stopping it afterward | No `/wrap-up` cleans up after a standalone run — Step 3.5 must stop any ephemeral server it started, per `_shared/dev-url-detection.md`'s "Standalone" cleanup rule. |
