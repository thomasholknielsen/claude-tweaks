...
**Advisory path — unproven by fixture; a concrete discrepancy found instead.** Rule 2's advisory-only case is established from upstream source (`main.mjs`'s `process.exit(primary.length > 0 ? 2 : 0)` and its accompanying comment), not from a replayable fixture. Reading `registry/antipatterns.mjs` and `findings.mjs` explains why a fixture can't be built: the exit-code exemption and the `severity` field are independent flags that never agree at pinned 3.5.0. `isAdvisory()` (in `cli/engine/cli/main.mjs`) exempts a finding from the exit-code count only when its rule carries `advisory: true` — exactly one rule in the registry, `em-dash-overuse`. But `finding()` sets `severity: ap.severity || 'warning'`, and `em-dash-overuse` has no `severity` key, so its findings report `severity: "warning"` even while exempted. The other 11 rules carrying `severity: 'advisory'` (`blinking-cursor`, `numbered-section-labels`, `gpt-thin-border-wide-shadow`, and eight more) lack `advisory: true`, so they still count toward the exit-code gate. Verified live: a `gpt-thin-border-wide-shadow` fixture (1px border + 24px-blur box-shadow) produced one finding with `"severity": "advisory"` and exited **2**. No rule in the pinned registry can produce a fixture that is both "every finding `severity === advisory`" and "exit 0" — those two signals disagree on every rule that carries either one. This does not weaken rules 2 or 5 above: the wrapper already derives pass/fail from the parsed `severity` field, never the exit code, so it is unaffected by which internal mechanism the CLI uses. It does mean "advisory-severity issues exit 0" is imprecise shorthand — the accurate statement is narrower: the one rule actually exempted from the exit-code count reports `severity: "warning"`, not `severity: "advisory"`.
