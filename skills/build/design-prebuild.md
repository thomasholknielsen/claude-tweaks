...
**Surface source:** `surface` (the gate condition SKILL.md's Common Step 1.7 checks before invoking this file's procedure) comes from the materialized header's `surface:` field — lifted from the record body's `Surface:` metadata line per `skills/flow/materialize.md`'s Surface/Design-intent lift rule. `design-intent` lifts from the same header/body source but is consumed downstream by `/claude-tweaks:design-wrapper polish`, not by this pre-build gate.
