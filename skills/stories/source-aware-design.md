...
4. **STALE** stories (locators unresolvable OR source files modified OR behavioral contracts changed): **REGENERATE** with updated locators from the current snapshot, refreshed source_files from the URL-to-Source-File Mapping, and updated SourceContract data from Step 1.5.
