...
- **Layer 2 (the record's `Surface:` body-metadata line, lifted into the materialized build header — spec 20)** — applies only when a spec is resolvable from the file list. `Surface: backend|infra` returns skip.
