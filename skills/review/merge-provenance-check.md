...
Read from `SKILL.md`'s Step 2 only when the detect command there reported at least one merge
...
"The change analysis below" is the diff-shape analysis in `SKILL.md`'s Step 2, which resumes after this check; Step 5 (`/claude-tweaks:simplify`) takes the same own-work scope.
