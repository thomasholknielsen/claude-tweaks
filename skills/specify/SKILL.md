...
| Granting or touching `auto:*`/`bot:*` from `/specify` | Authorization is human-granted only (`/backlog refine`'s territory); bot-state is machinery's. `/specify` adds `ready`, `risk:*`/`effort:*`, Type when absent, removes `parked` on promotion, touches neither family (matrix in `_shared/work-record.md`). |
