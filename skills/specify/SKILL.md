...
3. **Cross-Spec Promises** (only when this decomposition met `promise-register-min-leaves` — default `4`, `_shared/work-record.md`'s Config keys table) — add a `## Cross-Spec Promises` section to the **parent** body, recomposed alongside Decision Rationale and the task list. The whole mechanism is `work-backend: github-issues` + `work-links: body-text` only — it depends on the `Blocked by #N: {assumption}` body-line convention, which doesn't exist under `work-links: native` (relationships live in GitHub's dependency graph, no body text to carry an assumption) or `work-backend: local-files` (frontmatter `blocked-by` arrays have no field for assumption text); a decomposition under either of those never gets a `## Cross-Spec Promises` section, regardless of leaf count. Seed one row per `Blocked by #{blocker}: {assumption}` line the Linking pass above just wrote between two leaves of this decomposition — `{blocker}` is the same number from that line (the record being depended on); `{owner}` is the dependent leaf whose body carries the line (pre-existing-record links don't get a row — the register tracks promises within this family, not every dependency):
