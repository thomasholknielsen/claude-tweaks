...
> **QA note:** Stories are auto-generated when `/flow` detects UI file changes, and validated by `/test qa` (or automatically in the `/flow` test step). Browser integration (below) is needed for story generation and validation.
