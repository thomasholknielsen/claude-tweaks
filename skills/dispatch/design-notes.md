...
## Why dispatch's own run dir is never a dispatched group's run dir

Each dispatched group's `/claude-tweaks:flow` invocation creates a separate, later `PIPELINE_RUN_DIR` of its own for the actual pipeline execution; dispatch's standalone-auto run dir exists only to hold this firing's `decisions.md`. The two are never the same directory, which is exactly why Step 5 threads `CLAIM_RUN_ID` explicitly into the Task agent rather than relying on `/flow` inheriting dispatch's run id.
