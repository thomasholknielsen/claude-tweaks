...
**Validate `{run-dir}` before substituting it — never pass the captured string through unvalidated.** The value derived in section 1 must be an **absolute path anchored under the main checkout's `.claude-tweaks/pipelines/`** — it must prefix-match the run root derived per `_shared/pipeline-run-dir.md`'s Anchoring section, i.e. it resolves under `$RUN_ROOT/.claude-tweaks/pipelines/`, where `$RUN_ROOT` comes from that section's `git rev-parse --git-common-dir` snippet, never from the worktree's own cwd. A relative `{run-dir}` is exactly the failure this check exists to catch: substituted unvalidated, the second call's fresh agent resolves it against its own worktree cwd instead of the main checkout, and `decisions.md`/`config.yml`/`staged/` are created inside the worktree and silently destroyed at teardown. **On failure** (the captured value is relative, or doesn't anchor under `$RUN_ROOT/.claude-tweaks/pipelines/`), do not substitute it as-is — re-derive `$RUN_ROOT` via that section's Bash snippet and rebuild `{run-dir}` as `$RUN_ROOT/.claude-tweaks/pipelines/{basename of the captured MANIFEST path's run directory}` before substituting.

So the dispatching session hands the run over explicitly — the validated `{run-dir}` is substituted into the second call's command line, inline beside the existing `CLAIM_RUN_ID`, in the literal prompt `task-prompt.md` inlines verbatim:
...
*(A third terminal path exists alongside this one, `integration-model: local-merge` only: the second call succeeds and reports `OUTCOME: ready-to-merge`. That path is owned entirely by `settle-and-merge.md`'s **Dispatching-session merge execution (local-merge fallback)** section, not this file — it needs the run-dir and worktree/branch values this file already resolves, but its own procedure (merge, push, then the cleanup this call deliberately deferred) is specified there. Under `integration-model: pr-first` (`_shared/integration-model.md`), this path doesn't exist at all — the second call merges itself via `_shared/pr-first-merge.md` and reports `merged`/`armed`/`pending-review` directly, so there is nothing left for the dispatching session's own thread to do on a success path.)*
...
2. **Worktree teardown still routes through wrap-up.** `[IL-116]` forbids calling `ExitWorktree` / raw `git worktree remove` on a pipeline-run worktree: it skips `wrap-up/cleanup-procedures.md` Section C step 3.5's transitional guard and can permanently destroy a pre-anchoring run's `config.yml`/`decisions.md`/`staged/` with no git history to recover from. But wrap-up lives only in the second call, which on this path was never dispatched. So the dispatching session makes one further `/flow` call, from inside the group's worktree, purely to reach that cleanup route:
   ...
   **`wrap-up` alone, not `review,wrap-up`.** `flow/steps-and-gates.md`'s Gate Behavior table gives `review` a **STOP** on any non-PASS verdict, so a step list leading with `review` after a failed build would stop before wrap-up and never reach cleanup at all. `wrap-up`'s own gate "always passes," which is what makes it usable as a cleanup-only invocation.
   ...
   `{run-dir}` here is the same value section 3 already validated (absolute, anchored under `$RUN_ROOT/.claude-tweaks/pipelines/`) — this call does not re-derive or re-validate it. When section 4 could not derive a `{run-dir}` at all, omit `PIPELINE_RUN_DIR` from that call — the teardown still runs through the sanctioned route, and the unresolvable run dir is named in the group's report rather than papered over.
