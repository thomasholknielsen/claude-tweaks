...
Each group's two `Task()` prompts are defined in `task-prompt.md` in this skill's directory — read it and inline each call's content verbatim into its own `Task()` tool call (per `_shared/subagent-output-contract.md`'s input discipline: minimal input, literal output template inlined, no conversation history — and that same `_shared/subagent-output-contract.md`'s fan-out section for the general single-message-emission rule, which this skill's group loop satisfies trivially since it dispatches one `Task()` at a time by the structural cwd-pinning constraint above, not a batch). Do not paraphrase or summarize either template; the exact wording is load-bearing for the four-value status line and output format contracts downstream skills parse.
