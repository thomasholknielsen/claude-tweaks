...
| `_shared/work-record.md` | Taxonomy home — the label contract and the permission-matrix row dispatch implements (`bot:in-progress` / `bot:blocked` add; `auto:merge` / `auto:*` / `bot:in-progress` remove; never add `auto:*` or `ready`). |
| `_shared/work-record-config.md` | Config-key home split out of the taxonomy — Preflight reads `work-backend` and the `backlog-backend` legacy-alias exception there, and it is the canonical home of this skill's own `## Configuration` table's four rows (`dispatch-retry-ceiling`, `automerge-max-lines`, `automerge-max-files`, `dispatch-pick-max-concurrent`); see that section's own cross-reference. |
