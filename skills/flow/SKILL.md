...
**Creative Opportunities survey (v4.5.0).** Before rendering the summary, and when `no-creative` was not set (nor `creative-survey: off` in CLAUDE.md), run decline detection (compares prior recommendations cache against the new diff to suppress repeatedly-declined items), then invoke `/claude-tweaks:design-wrapper survey <changed-files>`. Returned recommendations render as a Creative Opportunities block (template below) before Next Actions; empty or `{skipped}` returns omit the block. When `no-creative` is set, skip decline detection and the survey call entirely and omit the block — mirrors `no-deepen`'s handling of the Depth Opportunities survey below.
...
| `/claude-tweaks:specify` | Produces the `ready` leaf records `/flow` accepts as `#N`/`#A,#B`, materialized via `materialize.md` — flow never calls `/specify` internally; an unshaped record's materialization hard gate points back here instead. Also creates the legacy numbered specs the alias path still reads. |
...
| `/claude-tweaks:assess-agent-autonomy` | Calls `ceremony-check` inline (not a fresh Task dispatch) only as a **fallback**, for a record reaching materialization (`skills/flow/materialize.md`) with no `ceremony:*` label — `/claude-tweaks:specify` is the mode's primary caller and sole label owner; the fallback verdict populates that run's own materialized header field without writing a label back, later folded into the `ceremony-profile` Manifesto lever. |
