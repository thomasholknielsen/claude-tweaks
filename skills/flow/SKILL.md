...
**Adopt-if-set, before creating:** a `PIPELINE_RUN_DIR` set on entry, naming an existing anchored directory, is adopted (nothing created or re-initialized, levers read from its `config.yml`); set-but-missing, unanchored, or unset creates as below. Branch: `steps-and-gates.md`'s **Adopting an inherited run directory**.
