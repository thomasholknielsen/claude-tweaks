...
4. **Pass context forward** — each step's output feeds into the next:
   - Step 2.5 → `build` receives `MERGE_CHECK_PASSED=true UPSTREAM_SHA={sha}` when Step 2.5 ran (worktree strategy + `merge-check: true` — see `validation.md`'s "Memo stamp" note). `build/worktree-setup.md`'s own Pre-flight merge check consults this and skips its otherwise-duplicate fetch-and-compare when the stamped upstream sha still matches. Absent (standalone `/build`, `current-branch` mode, or `merge-check: false`) → `/build` runs its own check as documented, fail-open.
