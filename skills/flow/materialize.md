...
Sequence, in `worktree` mode (the default): create the worktree (`/build`'s Common Step 1, or `/flow`'s own up-front shared-worktree creation for a multi-record run), then create `{run-dir}/work/` inside it, compose the file, and commit it there as the branch's first commit.
