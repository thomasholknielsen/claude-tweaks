...
1. For each `spec-{N}/staged/` patch, in spec order: apply per `_shared/staged-patch.md`'s Console apply with description fallback — `git apply --check` then `git apply` against the **cumulative pipeline state** (every earlier spec's commits, plus this run's simplify/polish/fix-wave commits, are already in the tree). A diff staged during spec N's review is expected to be stale by the time spec N+k's phases have run; on a failed check, read the artifact's `Target:` (an explicit repo-relative path, so cumulative drift stays resolvable) and `Invariant:` and re-derive the edit directly in the current tree, logging which path applied to the originating spec's `decisions.md`. Description-only `staged/{slug}-{n}.md` items (patches that failed the staging-time gate) go straight to re-derivation; an item that cannot be re-derived stays `open` and renders in the "Not run / Failed" footer with its reason — never dropped silently.
