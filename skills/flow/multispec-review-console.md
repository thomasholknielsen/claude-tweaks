...
<!-- local-merge-fallback --> `local-merge` never populated this either — the old procedure required `CLAIM_RUN_ID` (dispatch-only) and a forge. A `local-merge` bundle that parks just stays resident in its session.
