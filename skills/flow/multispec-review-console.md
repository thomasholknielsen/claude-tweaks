...
3. For each `M#` memory update: same Approve all / Override handling as `Q#`. On Apply/Edit: write the memory file and append its `MEMORY.md` index line per `_shared/learning-routing.md`'s D4 procedure, at batch approval or auto-resolution — same mechanism as the single-spec console (the write lands outside the repo, not in this commit). Skip (Override only) drops the proposal — log the decline the same way as `Q#`.
