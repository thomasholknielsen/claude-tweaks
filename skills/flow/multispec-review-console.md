...
**Canonical render order.** This is the one place this order is stated — every other reference to it (step 4 above, the template below) points here rather than restating the list: Auto-applied, Pending review, Low-confidence findings, Contested findings, then the five engine-rendered sections in their own position, then Cleanup actions, Issue closures, Translated briefs, Queue writes, Refused — no defer reason (`wrap-up/refused-proposals.md`; renders only when non-empty, no sequence, no default), Memory updates, Upstream feedback.
