...
0. **If `MULTISPEC_CURATION_DEFER=1` was set**, run the batch-scope reflect + registry pass now, before step 1 below — see `multispec-batch-curation.md` in this skill's directory. It writes findings into the parent run dir's own `decisions.md`/`staged/`/`engine-state.json`, which step 2's reads and step 3's engine call below already cover once it has run.
