...
4. Run the pending-review branch durability step below, then render the consolidated console (template below): the prose-aggregated sections from step 2's reads, then the engine's verbatim output from step 3 in its own position (see the template), then the remaining prose-aggregated sections in the order "Numbering rules" below states.
...
Run this before rendering the consolidated console below, never after. **Gate the read.** Read `_shared/pending-review-durability.md` only when `CLAIM_RUN_ID` is set and non-empty (dispatch-originated), this run used a worktree strategy, and this `/flow` invocation's step list contained `review` and that step passed for every spec that ran. Otherwise skip this section entirely and do not read the file. Full procedure — the bundle push/PR rule, where the outcome record lands and how push/PR failures are carried per-record, and the row-numbering interaction between the engine-fed and prose-aggregated sections ("Two provenances, one sequence") — is in `pending-review-durability-console.md` in this skill's directory.

## Auto-resolution short-circuit (`consoleAutoResolve`)

Mirrors `wrap-up/review-console.md`'s own Auto-resolution short-circuit, aggregated across every spec in the run — this is the path a dispatched bundle group reaches when `/claude-tweaks:dispatch` invokes `/flow "#A,#B"` under an `unattended` ceiling (`dispatch/SKILL.md` Step 5); without it, this console would render the same blocking `AskUserQuestion` a headless firing never returns from that the single-spec console's own tier split protects against.

Resolve the ceiling once — `CEILING=$(node "${CLAUDE_PLUGIN_ROOT}/bin/resolve-policy.js" --values autonomy)` — and check `bookkeepingPermissions(ceiling).consoleAutoResolve` (`bin/lib/issues/autonomy.js`, granted at `unattended` only — see `_shared/autonomy-ceiling.md`). When **not** granted, skip this section entirely and proceed to "Present the consolidated console" below — the ordinary `auto`/`hybrid` path, unchanged. When `--dry-run` was also passed, its preview-only behavior takes precedence regardless of ceiling — nothing here executes a real write.

When granted, render the consolidated console as an **informational report** instead of a prompt: every section from "Present the consolidated console" below still appears, aggregated across every spec — nothing is dropped or summarized away. Then resolve every item per its own stated default, with **zero** `AskUserQuestion` calls, exactly as the single-spec short-circuit does:

- Every batch-section item (Auto-applied through Translated briefs) resolves as if "Approve all" had been chosen.
- Every `Q#`/`M#` item resolves to `Apply` — its pre-checked default.
- Every `U#` item resolves to **filed**, not its usual unchecked/declined default — the same one-exception rule the single-spec console's short-circuit states (`_shared/autonomy-ceiling.md`'s Notification section; #347's Decision Rationale).

Execute each resolution via "On approval" below. Two differences from a human-driven "Approve all": log one `AUTO {time} — Review Console: auto-resolved {item}. Reversibility: {…}.` line per item to the originating spec's `decisions.md` (or the parent run dir's, for a parent-level item) instead of relying on a user answer to imply it, and **retain every `staged/` file** rather than deleting/consuming them on apply — they stay as revert artifacts. Send **one** consolidated `PushNotification` for the whole multi-spec run — not one per spec, not one per item — at the same point the single-spec auto-merge short-circuit sends its own FYI (`_shared/autonomy-ceiling.md`'s Notification section).

After resolving, proceed directly to Cleanup actions execution (Shared teardown below) and archive the parent run dir — skip "Present the consolidated console" and its `AskUserQuestion` call entirely.
...
2. For each `Q#` queue write: choosing Approve all resolves it to its pre-checked `Apply` default directly — no separate prompt; choosing Override instead prompts the user per item via its own `AskUserQuestion` call (see "Present the consolidated console" above) — never batched, even across specs. On Apply (or Edit, after the modification): create the record — `gh issue create` (`work-backend: github-issues`) or `local-store.js`'s `writeRecord` (`work-backend: local-files`), reading `Title:`/`Type:`/`Labels:` and the body from the item's staged record-proposal file (`leftover-*.md`, `ledger-record-*.md`, or any other `Title:`-headed staged proposal — the same generic clause `wrap-up/review-console.md`'s create step uses). Skip (Override only) drops the proposal — log the decline to the originating spec's `decisions.md` (or the parent run dir's, for a parent-level leftover) with the user's stated reason, or "declined, no reason given" when none was offered.
3. For each `M#` memory update: choosing Approve all resolves it to its pre-checked `Apply` default directly — no separate prompt; choosing Override instead prompts the user per item via its own `AskUserQuestion` call (see "Present the consolidated console" above) — never batched, even across specs. On Apply (or Edit, after the modification): write the memory file and append its `MEMORY.md` index line per `_shared/learning-routing.md`'s "Memory write procedure (D4)" at batch approval or auto-resolution, reading the proposed file and index line from the item's staged file (aggregated across every spec's `staged/wrap-up-memory-*.md` files plus the parent run dir's own). The memory directory comes from the invoking assistant's own system prompt — never derived or guessed. This write lands outside the repository, so it is not part of the multi-spec wrap-up commit below. Skip (Override only) drops the proposal — log the decline to the originating spec's `decisions.md` (or the parent run dir's, for a parent-level item) with the user's stated reason, or "declined, no reason given" when none was offered.
4. For the `U#` upstream feedback rows (aggregated across every spec's `staged/wrap-up-upstream-*.md` files plus the parent run dir's own): choosing Approve all resolves every row to declined directly — no separate prompt, nothing filed (the Auto-resolution short-circuit below is the one exception); choosing Override instead presents them via `_shared/upstream-feedback-batch.md`'s shared batch contract (see "Present the consolidated console" above) — the body rendered in that table (read from each item's staged file when the table was built) **is** the approved snapshot. For each item checked in the resulting chunk(s): invoke `/claude-tweaks:feedback --pre-confirmed` passing both the staged-file path and that approved snapshot, distinct from the fresh re-read `feedback/SKILL.md`'s Step 7 does on its own — its drift check diffs the two, and its Step 6 scrub always reruns regardless as a separate safety net; on drift it falls back to its own normal `AskUserQuestion` confirm for that one item. An item left unchecked (Override) or resolved by the Approve all default is declined per the shared contract's decline rule — log to the originating spec's `decisions.md` (or the parent run dir's, for a parent-level item) with the user's stated reason, or "declined, no reason given" when none was offered.
...
- **Queue writes, Memory updates, and Upstream feedback are covered by the terminal Approve all / Override / Stop decision**, mirroring `wrap-up/review-console.md`'s single-spec stance exactly. Approve all resolves every `Q#`/`M#` item to `Apply` and every `U#` item to declined — their own stated defaults — with zero additional `AskUserQuestion` calls. Override is what still drills per item: `Q#`/`M#` one `AskUserQuestion` call each, never two items from the same section (even from different specs) batched into one call; `U#` via `_shared/upstream-feedback-batch.md`'s chunked `multiSelect` calls aggregated across specs. This enforces `_shared/auto-mode-contract.md`'s not-silenced rules for work-record creation, memory writes, and upstream filing by construction — Approve all still applies each item's own stated default, Override still resolves each item individually, and no per-item gate can be bulk-resolved by a shared toggle either way.
