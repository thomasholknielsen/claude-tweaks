...
Read and parse the resolved template file's content (`template_version`, `model`, `allowed_tools`) now — Steps 3 and 3.5 below assume this has already happened, exactly as the per-skill path's own Step 1 already does. Also read the schema's current `kernel_version` once per run (`grep -m1 '^kernel_version:' skills/_shared/routine-template-schema.md`) — Step 3 below reuses this single value for every record's kernel-staleness check rather than re-invoking the grep per row.
