...
| `/claude-tweaks:flow` | Does not ship its own `routine-template.yml` — `/flow`'s own Relationship table points here instead: `/routine create dispatch` instantiates the scheduled headless dispatcher that runs `/flow`'s pipeline (see the `/claude-tweaks:dispatch` row above), so `/flow` has no direct routine of its own to create. |
