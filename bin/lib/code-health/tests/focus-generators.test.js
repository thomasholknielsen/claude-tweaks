...
const { FOCUS_GENERATORS, registerGenerator } = require('../focus-generators');
...
test('FOCUS_GENERATORS: the registered "dead-code" generator is the same function candidates-dead-code.js exports as scanDeadCode, not a copy', () => {
  const { scanDeadCode } = require('../candidates-dead-code');
...
test('the other require order (candidates-dead-code.js required first, in a fresh module cache) also yields a populated registry', () => {
  ...
  delete require.cache[require.resolve('../candidates-dead-code')];
  delete require.cache[require.resolve('../focus-generators')];
  const { scanDeadCode } = require('../candidates-dead-code');
  const { FOCUS_GENERATORS: freshRegistry } = require('../focus-generators');
