...
- **An absence assertion over an `indexOf`-pair slice needs a presence assertion on the same slice, in the same test.** `text.slice(text.indexOf(START), text.indexOf(END))` silently yields `''` when the start anchor moves — `slice(-1, n)` is empty, not an error — so a lone `assert.ok(!region.includes(X))` certifies an absence it never looked for. This is the "empty extraction passes the sweep for free" blind spot in its non-regex form, and the idiom is repo-wide: twelve call sites across six suites (`grep -rn '\.slice(.*indexOf(.*indexOf(' tests/`), none of them guarding the indices. The two carrying an absence check survive only *incidentally*, by a sibling `assert.match` on the same region in the same test (`tests/curation-judge-stagepath.test.js`, `tests/staged-patch-contract.test.js`). `tests/materiality-floor-conformance.test.js`'s `proactive-sweep is deliberately NOT a DEFER_REASONS member` (#1279) is the unguarded instance — the negation is its only assertion, and its end anchor is the file's *first* `']);'` rather than the one after the start, so a frozen array added above `DEFER_REASONS` in `plugin/bin/lib/issues/record.js` truncates the region silently as well. Assert the region non-empty, and anchor the end on the start index (`indexOf(END, start)`).
