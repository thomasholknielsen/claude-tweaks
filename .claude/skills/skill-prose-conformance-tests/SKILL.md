...
| Adding a new decision-log doc bullet next to an existing one without checking whether it widens an existing `decisionLogPattern` lookup | A pin test keyed on a short phrase (e.g. `'Findings present in both agents'`) can start matching the new bullet too, silently degrading its discrimination without going red; add a second, more specific `mustInclude` term to restore it (near-miss, spec #733, 2026-08-26) |
