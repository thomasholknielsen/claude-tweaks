...
description: Use when writing or reviewing a plugin/bin/lib/ module or CLI in this repo that shells to `gh` — the injectable-runner seam, the three gh api value mechanisms (-f/-F/path placeholders), per-call fail-safe batching, and the CLI deps contract. Keywords - gh api, runner, execFileSync, GraphQL variables, -f, -F, placeholder, injectable, fake runner.
---
