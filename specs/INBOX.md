...
## Reconcile documented vs. actual worktree directory convention

**Added:** 2026-07-08 | **Category:** technical | **Related:** skills/init/bootstrap-steps.md (Step 6)

Context: `bootstrap-steps.md` Step 6 documents `.worktrees/` at the project root as "the standard worktree directory... matches superpowers v5.1.0's preferred path and is the only directory `/superpowers:finishing-a-development-branch` will clean up." But the native `EnterWorktree` tool actually creates worktrees under `.claude/worktrees/` — confirmed live in this repo, which currently has multiple active worktrees under `.claude/worktrees/` and none under `.worktrees/` (empty). Discovered incidentally while using `EnterWorktree` for an unrelated feature.

Scope: Determine which convention is actually correct going forward — update `bootstrap-steps.md` to document `.claude/worktrees/` as the real native-tool convention (and reconsider the migration guidance that currently treats `.claude/worktrees/` as legacy), or confirm `.worktrees/` remains correct for the git-fallback path (`git worktree add` per `using-git-worktrees` Step 1b) while `EnterWorktree`'s native path is a separate, harness-owned convention that doesn't need reconciling. Needs investigation into whether `finishing-a-development-branch`'s cleanup step actually handles `.claude/worktrees/` worktrees correctly today, since its own docs say it only cleans up `.worktrees/`.
