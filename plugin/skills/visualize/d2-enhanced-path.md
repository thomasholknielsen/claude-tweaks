...
Scale the source to `SKILL.md` Step 1's density tier, the same rule the baseline path's Step 4 applies. **Minimal** — construct as above, short node labels, minimal container nesting. **Rich** — add a one-line description as a second label line inside each significant node (D2 supports multi-line labels), group related nodes into named containers that carry their own explanatory label rather than a bare box, and add a standalone legend container off to one side when the diagram's shapes/edge styles aren't self-explanatory. Keep every addition inside D2's own construct vocabulary — don't hand-author raw SVG detail here; that only happens in Step 3's re-theming pass, never as a workaround for something D2 itself can express.
