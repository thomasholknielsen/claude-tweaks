...
**Transcript-judge integration (record #857).** The standalone singleton's artifact bundle above is joined by `_shared/transcript-judge.md`'s shared dispatch harness, consumer key `reflect`. The dispatch prompt additionally carries: the resolved transcript path, immediately followed by the shared contract's cross-session coverage-limit scope statement (main session's own transcript only — a dispatched Task agent's work is a named coverage gap, not silently included); the slicing guidance; and — when `readWatermark(path, { consumer: 'reflect' })` returns non-null for the resolved transcript path — `formatOffsetClause(...)`'s literal output as the conditional offset-clause item. Model profile stays `frontier` (parameterization point 3 — unchanged from the singleton dispatch above); watermark consumer key is `reflect` (parameterization point 4). `dismissedSubjects` for that item 5 (#1033, mirroring `session-evaluation.md`'s fix for feedback) is `bin/lib/declined-learning/store.js`'s `listDeclined({ source: 'wrap-up' })` mapped to each entry's `subject` text (falling back to the bare fingerprint for a pre-#1033 entry with none) — computed live, immediately before composing the offset clause, never read off the watermark object `readWatermark` returned, for the same one-run-lag reason `session-evaluation.md` documents. Before this fix, reflect's own judge dispatch never populated this field at all, so item 5 always rendered "previously declined findings about: none" regardless of how many insights a human had actually declined via the "Don't capture" resolution.
