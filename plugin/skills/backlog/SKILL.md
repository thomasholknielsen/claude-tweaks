...
- A record hit its retry ceiling (`bot:blocked`) and needs a human's renewed judgment before it can re-enter the autonomous queue, or was parked by the merge-verification gate (`bot:parked`, grants intact) and needs its PR checked — `refine` mode.
