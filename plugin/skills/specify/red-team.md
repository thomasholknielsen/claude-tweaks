...
7. **Decision-log entries — only when a run directory resolved.** When step 5 found one (steps 1-4 of `_shared/pipeline-run-dir.md`'s Resolution order): one `AUTO` summary per sub-issue record, never one entry per finding, plus one `STAGED` entry per finding actually staged:
   ```
