...
| **Delete** (legacy-root `artifact` finding, `remedy: record` — a pre-relocation screenshots/ or traces/ tree with content fresher than 30 days, or one git could not prove untracked) | Stage | Stage | Stage — deleting possibly-fresh or possibly-owned content is never auto-applied at any tier; the probe emits `remedy: record` precisely because ownership or age confidence is missing (`bin/lib/residue/probes/artifacts.js`) |
