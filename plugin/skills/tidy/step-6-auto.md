...
| **Re-triage (parked)** (Shape 5.6 — `bot:parked`; no mutation, recommends checking the PR's checks and resuming via `/claude-tweaks:dispatch` — grants are intact, so there is nothing here for `/claude-tweaks:backlog refine` to re-grant) | Auto (no-op, always surfaced) | Auto (no-op, always surfaced) | Auto (no-op, always surfaced) |
