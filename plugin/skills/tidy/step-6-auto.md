...
| **Delete** (any case requiring judgment, excluding backlog records — old plans whose spec status is unclear, design docs with no specs, legacy-root `artifact` findings carrying `remedy: record` (a pre-relocation screenshots/ or traces/ tree with content fresher than 30 days); see the dedicated backlog-record Delete rows below for `local-files`- and `github-issues`-backend findings) | Stage | Auto-apply | Auto-apply |
