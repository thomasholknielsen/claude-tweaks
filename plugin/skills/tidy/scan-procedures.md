...
- **One or more referencing records found:** the same staged proposal additionally lists one Close row per referencing record, using tidy's Close (GitHub) shape (`Finding`/`Proposed`/`Commands` — see the worked single-target example in Step 5's Action Vocabulary), collected under a `Rows:` section alongside the ledger delete itself, and the `Commands:` section includes both the `git rm {ledger path}` and each referencing record's `gh issue comment`/`gh issue close` pair. `tidy --approve` applying this one proposal therefore deletes the ledger and closes every record naming it in the same action. No ledger is ever recommended Delete while a referencing record stays open and unproposed.
