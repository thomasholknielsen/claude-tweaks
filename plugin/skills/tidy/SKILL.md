...
`/claude-tweaks:tidy` has one sanctioned parent: `/claude-tweaks:sweep`, whose Step 1 passes `--source sweep` — the explicit parent signal; there is still no `PIPELINE_RUN_DIR` signal expected (a set `$PIPELINE_RUN_DIR` alone never means parent invocation — the run dir is resolved internally for Step 6's routing). Under `--source sweep`: auto-mode runs regardless of `auto-mode` policy, entries log to the shared `decisions.md`, findings stage to the shared `staged/`, and both `## Next Actions` and `AskUserQuestion` approval are suppressed — counts report to the parent; staged items wait for `/claude-tweaks:tidy --approve`. Every other invocation is standalone: `## Next Actions` always renders.

One exception to "never creates or enters a worktree": under `worktree-always: true`, Step 7.5 sets up a scratch worktree to satisfy the PreToolUse gate for Step 7's mutations and the tidy-up commit, then merges back and tears down before Next Actions render — see Step 7.5's `worktree-always` handling above. Bookkeeping internal to Steps 7-7.5, not a parent-skill relationship.
