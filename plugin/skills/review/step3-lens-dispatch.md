...
> - Findings present in both agents' outputs at the same location but rating severity in different buckets (e.g. high vs medium) → still emit as `confirmed`, with the confirmed severity taken as the lower of the two (`categoriseReproduction`'s `severityContested: true` output — #733: a pair that agrees on location straddling the bucket boundary is a real reproduction, not a false negative). Write: `AUTO {HH:MM:SS} — Reproduction: lens "{lens}" finding {path}:{line} reproduced, severity contested ({severityA} vs {severityB}; confirmed as {severity}). Reversibility: high.`
