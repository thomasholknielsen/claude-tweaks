...
| `absorb:N` | Appends `## Absorbed: {YYYY-MM-DD} — {title}` under record N's existing sections (never rewriting content above), delete the absorbed record's file | Per the Absorb mechanics below, then comment `Absorbed into #N.`, then `gh issue close {n} --reason "not planned"` — mirrors `/claude-tweaks:tidy`'s Absorb action |

**Absorb mechanics** (`github-issues`): appends `## Absorbed: {YYYY-MM-DD} — {captured title}` under `#N`'s existing sections, never rewriting content above, composed once via `_shared/github-write-transport.md` (`gh issue edit {N} --body-file`); past 55,000 post-append chars (vs GitHub's 65,536 cap), comment instead. Re-judges `size:` per `_shared/work-record.md` — raise only, never lower; `priority:*` stays unwritten, suggest higher priority in output. Output names target + append; invalidates the session snapshot per `_shared/record-queue-fetch.md`.
