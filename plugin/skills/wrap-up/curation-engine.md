...
if [ -z "$RUN_DIR" ] || [ -z "$WT" ]; then
  ...
  if [ ! "$SHADOW" -ef "$RUN_DIR" ] && [ -f "$SHADOW/decisions.md" ] && [ ! -L "$SHADOW/decisions.md" ]; then
    if SHADOW="$SHADOW" RUN_DIR="$RUN_DIR" node -e '
      const fs = require("fs");
      const lines = fs.readFileSync(process.env.SHADOW + "/decisions.md", "utf8").split("\n").filter((l) => l.startsWith("- "));
      if (lines.length === 0) process.exit(1);
      fs.appendFileSync(process.env.RUN_DIR + "/decisions.md", lines.map((l) => l + "\n").join(""));
    '; then echo "relocated: decisions.md (entries appended)"; else echo "sweep: shadow decisions.md had no entries — dropped"; fi
