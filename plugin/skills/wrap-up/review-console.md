...
5. Apply config updates (item 15: CLAUDE.md, rules, ADRs) — including any CLAUDE.md findings staged by the CLAUDE.md & rules curation row, which are always offered, never auto-applied. **Before applying, resolve every row belonging to `review-console-interactive.md`'s Hard requirements "Drills-individually class" instead of applying it directly here:** the `[adr-convention]` row's three-way `AskUserQuestion` first (its answer is global — resolves every blocked `[adr]` row in the run), then any restructural stage-only finding via `_shared/batched-item-drill.md`'s multiSelect chunking. Only after those drills resolve does the remainder of item 15 apply normally.
...
If `decisions.md` has zero **decision-bearing** entries (`AUTO` / `STAGED` / `KEPT-PROMPT` / `REFUSED` — `SCANNED` audit lines do not count; see below) AND `staged/` is empty AND there are no skill/config updates AND no cleanup actions apply AND no queue writes, memory updates, or upstream feedback proposals are pending, skip the console entirely. Log "Review Console: nothing to review" and proceed to the phase-trace report.

**`SCANNED` exclusion.** The curation engine (`bin/lib/wrap-up/engine-record.js`) writes one `SCANNED` line per registry row — closed rows at `plan` time, open rows at `record` time — so `decisions.md` holds `SCANNED` entries on every run, clean or not; counting them here would make this fast path permanently unreachable. `SCANNED` is an audit-scope line ("this row was looked at"), never a decision-bearing one — an open row that actually finds something writes its own separate `AUTO`/`STAGED` line alongside its `SCANNED` line, so a run with real findings still renders the console; only an all-`SCANNED` log (nothing applied, nothing staged) skips.

Cleanup rows that are unconditional bookkeeping — run-dir archival, `cleanup-procedures.md` item 8 — do **not** count as cleanup actions for this test; archival executes regardless, undisplayed. Without that carve-out the fast path could never fire, since item 8's condition now holds on every run. These two exclusions compose: a `decisions.md` holding only `SCANNED` lines, with only unconditional bookkeeping cleanup pending, still skips.
