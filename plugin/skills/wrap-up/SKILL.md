...
**Unrecognized-status warning (additive, non-blocking).** Before the Resolve Gate table, when `pack.ledger.unrecognized > 0`, render one warning line naming the count and the distinct out-of-enum values found (`pack.ledger.unrecognizedValues`) — e.g. "Warning: N ledger row(s) carry an out-of-enum status ({values}) — not counted as open, but not a recognized terminal value either; see `_shared/ledger-format.md`'s Status Lifecycle." This does not open or widen the gate — an out-of-enum row was never `open` and stays non-blocking; the line is visibility only, since such a row never appears in the `open`-only "Unresolved Open Items" table. Omit the line entirely when `unrecognized` is `0`.
