...
**When the dispatching session itself is a cwd-pinned subagent (#447).** Everything above assumes this dispatching session can freely enter each group's worktree. When `/claude-tweaks:dispatch` is itself invoked as a Task/Agent-tool subagent rather than a top-level session, this session's own cwd is pinned by the harness too, and its own worktree-entry attempt is refused for the identical reason a dispatched build/review call's `EnterWorktree` is refused — the same refusal signal, one level higher up the chain. Detect this before Step 5's loop starts by checking for that same cwd-pinned signal on this session's own context, not by waiting for a failed entry attempt. On detection: this session still creates the group's worktree with a plain `git worktree add` (a filesystem operation, not a cwd repin, so it works regardless of whether this session's own cwd can move), but since its own cwd cannot move into that worktree, it cannot rely on either of that group's two Task calls inheriting a worktree cwd from it — instead, every command those two calls' prompts issue is prefixed with an explicit `cd {worktree-path} &&`, so neither this session nor the calls it dispatches ever depend on a shared shell cwd nobody in the chain can set. A top-level dispatching session is unaffected — the inherited-cwd mechanism the rest of this step describes applies to it unmodified.
