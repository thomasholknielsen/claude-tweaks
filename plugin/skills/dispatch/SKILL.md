...
| `--batch-size <n>` (deprecated alias) | Deprecated alias for `--budget <n>` — same effect, one warn-tier notice per invocation. Removal condition: `deprecated-aliases.md`. The old suffix `#N,#M,...` use is no longer accepted — rejected exactly like `--budget` on an explicit list; explicit-list firings are capped by `dispatch-batch-size` alone. |
