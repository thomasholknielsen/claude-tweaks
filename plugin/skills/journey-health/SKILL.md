...
Write the combined Steps 2-3 findings array to this run's session-scoped `journey-health-findings-light.json` (`_shared/session-tmp-root.md`, cited throughout this file rather than restated; resolve via `eval "$(node "${CLAUDE_PLUGIN_ROOT}/bin/session-tmp-resolve.js" JOURNEY_HEALTH_FINDINGS_LIGHT=journey-health-findings-light.json)"` in whichever fence writes it). If neither step produced any findings, write `[]`.
...
**Digest-mode fold.** Before writing `$JOURNEY_HEALTH_ISSUES`, fold in any open digest issue's embedded checklist fingerprints per `_shared/health-filing-digest.md`'s GATHER-OPEN-ISSUES-step shape (`{PREFIX}` = `journey-health`) — this is what lets a previously-digested finding dedupe as a normal open-issue match in Step 5 rather than being re-judged or re-digested.
...
```bash
eval "$(node "${CLAUDE_PLUGIN_ROOT}/bin/session-tmp-resolve.js" JOURNEY_HEALTH_FINDINGS_LIGHT=journey-health-findings-light.json JOURNEY_HEALTH_PAYLOADS_LIGHT=journey-health-payloads-light.json)"
node "${CLAUDE_PLUGIN_ROOT}/bin/journey-health.js" validate-findings "$JOURNEY_HEALTH_FINDINGS_LIGHT" \
...
```bash
eval "$(node "${CLAUDE_PLUGIN_ROOT}/bin/session-tmp-resolve.js" JOURNEY_HEALTH_FINDINGS_DEEP=journey-health-findings-deep.json JOURNEY_HEALTH_PAYLOADS_DEEP=journey-health-payloads-deep.json)"
node "${CLAUDE_PLUGIN_ROOT}/bin/journey-health.js" validate-findings "$JOURNEY_HEALTH_FINDINGS_DEEP" \
...
```bash
eval "$(node "${CLAUDE_PLUGIN_ROOT}/bin/session-tmp-resolve.js" JOURNEY_HEALTH_RETRY_PAYLOADS=journey-health-retry-payloads.json)"
node "${CLAUDE_PLUGIN_ROOT}/bin/journey-health.js" retry-queue drain --root "${ROOT:-$PWD}" > "$JOURNEY_HEALTH_RETRY_PAYLOADS"
```

For each payload in `$JOURNEY_HEALTH_RETRY_PAYLOADS`, attempt `gh issue create` exactly as below. Track every attempt's outcome (retry-queue payloads AND any brand-new payload from this step's own filing loop that fails) as `[{ fingerprint, payload, ok: true }]` or `[{ fingerprint, payload, ok: false, error: "<gh's error output>" }]`, write to this run's session-scoped `journey-health-retry-results.json`, then re-resolve both session-scoped paths this fence needs (`_shared/session-tmp-root.md`; a fresh bash invocation does not inherit the prior fence's shell variable):

```bash
eval "$(node "${CLAUDE_PLUGIN_ROOT}/bin/session-tmp-resolve.js" JOURNEY_HEALTH_RETRY_RESULTS=journey-health-retry-results.json JOURNEY_HEALTH_ESCALATED=journey-health-escalated.json)"
node "${CLAUDE_PLUGIN_ROOT}/bin/journey-health.js" retry-queue update "$JOURNEY_HEALTH_RETRY_RESULTS" --root "${ROOT:-$PWD}" > "$JOURNEY_HEALTH_ESCALATED"
```

If `$JOURNEY_HEALTH_ESCALATED` is non-empty, file (or update) a `journey-health:filing-failed` issue for each entry, naming the stuck fingerprint and its failure history — bootstrap that label the same way as the others below.
