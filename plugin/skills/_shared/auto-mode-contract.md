...
| `/init` Step 6.5's port-rewrite diff gate (Apply all / Choose files / Skip) | Edits the user's own project config (`vite.config.*`, Compose files, etc.) — outside the always-reversible floor, the same category as work-record creation or memory writes. Runs in every mode including `auto`; a declined rewrite still sets `port-services` and runs the first `allocate` (`init/bootstrap/step-06-5-port-isolation.md`, #1794). |
