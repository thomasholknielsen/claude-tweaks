...
| `grant-veto-window-hours` | `policy.yml` | `/claude-tweaks:dispatch`'s Auto-merge gate (`dispatch/settle-and-merge.md`), `/claude-tweaks:wrap-up`'s Auto-merge short-circuit (`wrap-up/auto-merge-short-circuit.md`) | `24` | How long a machine-granted `auto:merge-pending` grant must sit unvetoed before either site matures it to `auto:merge` (#309) |
