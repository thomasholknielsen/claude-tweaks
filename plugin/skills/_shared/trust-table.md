...
```bash
node "${CLAUDE_PLUGIN_ROOT}/bin/fetch-sub-issues.js" $(node -e "require('{tmp-parent-issues}').forEach(p => console.log(p.number))") > "{tmp-sub-issues-batch}"
```
...
`{tmp-sub-issues}` or a stale-looking snapshot on disk.
