...
| **`/backlog refine`** (write mode, human present) | `auto:build`, `auto:merge` (human-confirmed), `priority:*` (human-confirmed via batch-apply), updates the `**Related:**` body line (human-confirmed), scoring supplied inline; posts a `<!-- backlog-refine-human-only -->`-marked audit comment (no label) on a scored `RECOMMEND_BUILD: false` record — `ready` and grants untouched | `ready` (flag back), `bot:blocked` (re-grant strip) | granting on a headless path, adding any `bot:*`, `risk:*`/`size:*` beyond the inline-override case, body-shaping beyond the `**Related:**` line, removing `ready` from a human-only-marked record |
