...
- **Refused:** two or more independent `$(...)` substitutions in one command (e.g., comparing `$(git rev-parse A)` against `$(git rev-parse B)` inside a `[ ]` test), `;`-separated sequences of top-level commands, any `for`/`while` loop (even with no filesystem access or all-read body), a single command using `${VAR:-default}` parameter-expansion-with-default syntax (distinct from the `$(...)` substitution case above — no chain or pipe involved), a `VAR=val && command "$VAR"` variable-assignment-then-use `&&` chain (distinct from an ordinary 2-command `&&` chain of ready-made values, which still passes).

**2026-08-25 addendum (#1108):** two further refusal shapes independently reproduced live in a worktree session, appended to the "Refused" bullet above rather than superseding the 2026-08-15 observation — `echo "${HOME:-/nope}"` refused while `echo "$HOME"` alone passes (isolating the `${VAR:-default}` shape from ordinary single-command variable expansion), and `TESTVAR=hello && echo "$TESTVAR"` refused while a bare assignment (`TESTVAR=hello`) and a plain 2-command chain with no assignment (`pwd && whoami`) both pass (isolating the assignment-then-use combination, not chain length, as the trigger).
