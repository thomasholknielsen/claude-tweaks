...
**Write mechanism (all modes, #1580):** compose all 13 lever values, then write them in **one call** — `node "${CLAUDE_PLUGIN_ROOT}/bin/set-config.js" --run "$PIPELINE_RUN_DIR" --set "mode={v},scope-creep={v},overlap={v},design-intent={v},leftover-default={v},auto-fix-threshold={v},review-auto-apply-ceiling={v},tidy-aggressiveness={v},ceremony-profile={v},model-stance={v},merge-verification={v},design-critique={v},merge-authorization={v}"` (all 13 `key=value` pairs, comma-joined, in the canonical lever order above) — never one `--key`/`--value` call per lever. The batch form validates the complete set against the canonical Manifesto lever list before writing any of it, the same fail-closed posture the single-key form already has (`bin/lib/set-config/write.js`). The single-key form (`--key <lever> --value <value>`) stays available for its one-lever callers — the ceremony escape hatch's downgrade-in-place write (`wrap-up/SKILL.md` Phase 1, `review/code-mode-steps.md`) — this is additive, not a replacement; `steps-and-gates.md` case 3 is *not* one of them: its recovery computes and writes the whole lever set ("same as case 5"), so it takes this batch form too, never 13 single-key calls. `spec:`/`created:` (the two bookkeeping keys in the yaml block below) are not levers — `--set`/`--key` both refuse them by design (the canonical Manifesto lever list excludes them) — so **this step writes them itself**, appending the two lines to the same `config.yml` after the batch call: no other step owns them, and `steps-and-gates.md` names neither key anywhere. Known gap: that append is an ordinary file write, so a worktree-pinned session — which has no Edit/Write path to the run dir, and no sanctioned writer that accepts a non-lever key — ends the run with both keys absent. Nothing reads either one today (`bin/resolve-policy.js` resolves POLICY_KEYS-backed levers only; `hooks.js`'s initialized-run-dir check tests `config.yml`'s presence, not its contents), so the gap is bounded — do not add a reader of `spec:`/`created:` without closing it first.
