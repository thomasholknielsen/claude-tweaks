...
**Log the absolute path written (#1826/#1810).** Immediately after the batch `set-config.js` call succeeds, log `AUTO {time} — Manifesto: levers written to {absolute path of $PIPELINE_RUN_DIR/config.yml}. Reversibility: high (config.yml can be rewritten by a later call).` — the same target-naming convention `record-pr`/`record-worktree` already follow for their own writes. Without this line, a run whose `config.yml` ends up missing or in the wrong place (a worktree-local shadow path, `[IL-127]`'s shape) leaves no way to tell "the write never ran" apart from "the write ran somewhere else" — exactly the gap #1810's occurrence hit, whose cause could not be established because this write logged no path at all.

**Log a `SKIP` on every path where this write does not run (#1826).** The Manifesto always writes in `auto`/`confirm`/`hybrid` mode on a fresh or minted-but-empty run directory (adoption cases 2-5) — the only two paths where the write is legitimately not performed are: (1) `interactive` mode, where Step 3 never runs at all (see "When to run" above); (2) adoption case 1 (an already-adopted run directory that already carries `config.yml`), where writing again would violate case 1's own "never re-initialize" rule. Both must write `SKIP {time} — Manifesto write skipped: {interactive mode, no Manifesto this run | case 1 adoption, config.yml already present at {absolute path}}. Reversibility: n/a.` per `_shared/auto-decision-log.md`'s degrade-trace rule — the empty-directory shape #1826 documents (a run that reached `test` with no `config.yml` and no explanatory line either way) must no longer be able to occur silently, whichever of these two legitimate skips it turns out to be, or a genuine bug in a third path this file didn't anticipate.
