...
target, exactly as before this CLI existed — read them from the composed `claims` bundle when a run directory already exists (`node "${CLAUDE_PLUGIN_ROOT}/bin/compose-context.js" --run "$PIPELINE_RUN_DIR" --step claims "${CLAUDE_PLUGIN_ROOT}/skills/_shared/issue-claims.md"`, then `$PIPELINE_RUN_DIR/context/claims.md`); Step 2.8 runs before Step 3 creates a fresh run's directory, so when no `$PIPELINE_RUN_DIR` exists yet, or if the compose command is unavailable or exits non-zero, read the named source files directly.
