...
**Template snapshot (#1840).** When the record's body carries a `Template: {path} @ {version}` line (written by `/claude-tweaks:harness-health` for a `claude-md`/`rule` finding in the `template-conformance`/`best-practice` categories — the Proposed block is a filing-time snapshot of that origin template at that plugin version, never a promise it still matches), the CLI reads the installed plugin's own version from `.claude-plugin/plugin.json` and includes the comparison in the JSON envelope as `template: { path, recorded, installed, changed }` (`template` itself is `null` when the record carries no such line — every finding except a template-derived `claude-md`/`rule` one, and every record filed before this stamp existed). When `changed` is `true`, the CLI also prints an actionable stderr line naming the path and both versions. The plan's first task should then re-read `{path}` at the *installed* version and derive the replacement from it — never apply the Proposed fence literally when a version delta is reported, or the commit closes one template drift and reopens another the next harness-health sweep re-files.
