...
| `test` | All checks pass — types, lint, tests, QA (when stories exist) — **and** the Lifecycle Stamp Gate (`test/SKILL.md` Step 1.6, [IL-131]) confirms `record-worktree` (and, under `pr-first`, the PR-early lifecycle) actually ran. `PASS_WITH_CAVEATS` counts as passed (caveats are informational). Sets `TEST_PASSED=true`. | Proceed to review | **STOP** — present test/QA failures, or the Lifecycle Stamp Gate's missing-stamp problem(s) |
