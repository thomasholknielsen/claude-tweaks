...
The distinction between case 1 and cases 2/3 is a single `fs.existsSync` check on `config.yml`; the distinction between case 2 and case 3 is whether the directory already carries other run content — `config.yml`'s absence alone is not evidence that nothing has happened yet (case 3 is exactly the counterexample).
...
**Phase exit (`worktree` mode, `integration-model: pr-first` — `_shared/integration-model.md`):** on every "Proceed to wrap-up" arrow above (skipped, no-work, or modified-and-passed) — push the branch and flip this phase's PR checklist row — `_shared/git-discipline.md`'s Phase-exit push section and `_shared/pr-early-run-lifecycle.md`'s Phase-checklist update section. A no-op under `local-merge` or `current-branch` mode. The row was never added to the PR body in the first place when polish is skipped for a structural reason (backend `surface:`, `no-polish`, `ceremony-profile: fast-lane`) — see that file's checklist composition rule — so there is nothing to flip on that path. A fast-lane run removes the polish row at its would-be exit exactly as `no-polish` does (`_shared/pr-early-run-lifecycle.md`'s skipped-phase rule) and does no phase-exit push — nothing landed.
