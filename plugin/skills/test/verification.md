...
When no file can be named (`retryDecision.reason === 'no-parse'`), "Isolating pre-existing failures by file" above does not apply — there is no path to isolate. Re-run the `tests` check once through the runner instead: `node "${CLAUDE_PLUGIN_ROOT}/bin/verify.js" --cmd tests="{command}"` with the same scope and `--no-stamp` (a second pass must never record a pass stamp on its own — `stamp.js`'s dirty-tree rule and #1921's stamp semantics stay authoritative; the adjudication is the agent's, the stamp is the runner's). Then adjudicate: the second run passes → **flake**, reported exactly as the named-file flake is (the run's verdict is unchanged from the pre-existing adjudication); the second run fails again → **regression**. One re-run, never more — the whole-suite cost is exactly why this path is the fallback, not the default.
