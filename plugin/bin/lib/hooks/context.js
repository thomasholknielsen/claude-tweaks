...
function resolveRun(cwd, env, sessionId) {
  ...
  for (const { dir, state } of iterRunDirsWithState(resolvedCwd)) {
    ...
    if (owner === me) {
      ...
      // `owner === me` here. classifyOwnership's worktree-binding check (via
      // isForeignWorktreeCandidate, #1410) is what actually separates them.
      // Skip only a provably foreign (sibling-bound) run and keep scanning;
      // 'mine' and 'indeterminate' both still return exactly as before —
      // fail-open, byte-identical for every single-session scenario (no
      // binding, or binding matches cwd).
      if (isForeignWorktreeCandidate(resolvedCwd, me, state)) continue;
