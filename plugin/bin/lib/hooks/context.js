...
//
// #1402: a fallback candidate is filtered on WORKTREE binding, via
...
// the caller's own repo/worktree can't be determined — e.g. cwd outside any
// repo, or the recorded worktree already gone from disk — so this never
// narrows the fallback beyond what #62's original "newest non-terminal, not
// session-foreign" guess already allowed for every candidate this file's own
// tests exercise without a real `worktree` binding). This is what stops the
// #1033 shape (concurrent sibling worktrees' events landing in an unrelated
// run's events.jsonl) without touching the 'env'/'session' arms above, or
// #413's exception, at all.
