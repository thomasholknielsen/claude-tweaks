...
5. **This repo's own `.claude-tweaks/policy.yml`** line becomes `worktree-always: true` in the same change (committed from the run's worktree — a worktree run is where this project's edits happen anyway).
...
- `node bin/resolve-policy.js --values worktree-always` resolves (default `false`); `--values worktree.always` returns `{ error: "unknown-key" }`.
