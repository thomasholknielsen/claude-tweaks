...
- Add a **conformance scan** step over the rendered report — a scan-row-shaped check the skill runs on the literal markdown it is about to send, *before* the existing "Hard gate (report before question)". Each row names the rule, the check, and the remedy: line length ≤ cap; every Yours row has exactly one `#{N} "{title}"` and one command line; no `(likewise …)` / `(also …)` / `(and N more)` shorthand anywhere; no annotation trailing a command on its line; Clean rendered as the fenced/comma form below. A non-conformant render is re-rendered, not shipped — the scan is a gate, not a note.
