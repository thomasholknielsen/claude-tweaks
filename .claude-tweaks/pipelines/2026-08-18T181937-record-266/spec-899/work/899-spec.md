...
**Build-time confirmation (2026-08-18):** #552 is CLOSED (completed) and its PR #880 is MERGED. This prerequisite is satisfied — proceed with the post-#880 file layout, locating sections by heading rather than the pre-split line numbers this record's Current State cites.
...
- **Build strictly after PR #880 merges.** Pre-build check, run before touching anything: confirm #552/#880 state. Then re-merge origin/main; the sections named above move in that PR. A conflict here is prose — the worst kind to merge — so starting from the post-split layout is mandatory, not advisory.
